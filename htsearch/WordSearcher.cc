...
//
bool WordSearcher::IsIgnore (const String & word)
{
  HtConfiguration *
...
WordSearcher::Fetch (const String & word)
{
  ...
  {
    if (debug)
      cerr << "REFERENCES: " << refs->Count () << endl;
    result = new ResultList;
    ...
    while (ref)
    {
      ...
      if (debug)
        cerr << "@ " << ref->Location () << endl;
      match->AddLocation (new Location (ref->Location (),
                                        ref->Location (), ref->Flags ()));
