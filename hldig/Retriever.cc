...
words (*(HtConfiguration::config ())), words_to_add (100, 0.75)
{
...
Retriever::parse_url (URLRef & urlRef)
{
  ...
  if (debug > 0)
  {
    ...
    // Display progress
    //
    cout << index++ << ':' << current_id << ':' << currenthopcount << ':' <<
      url.get () << ": ";
    cout.flush ();
  }
      ...
      if (doc->StoredLength () > 0)
      {
        if (check_unique_date)
        {
          ...
          {                     // Unknown date was set to current time
            md5 (bhash, doc->Contents (), doc->StoredLength (), &ddate,
                 debug);
          }
          else
          {
  ...
  case Transport::Document_not_found:
    ...
    if (debug)
      cout << " not found" << endl;
    recordNotFound (url.get (), urlRef.GetReferer ().get (),
...
//
void
Retriever::RetrievedDocument (Document & doc, const String & url,
                              DocumentRef * ref)
{
  n_links = 0;
  current_ref = ref;
...
Retriever::IsValidURL (const String & u)
{
  ...
  if (!badquerystr || !prevbadquerystr
      || prevbadquerystr->compare (bad_querystr) != 0)
  {
    if (!badquerystr)
...
Retriever::GetLocal (const String & strurl)
{
  ...
  while ((prefix = (String *) prefixes->Get_Next ()))
  {
    ...
    if (mystrncasecmp ((char *) *prefix, (char *) url, prefix->length ()) ==
        0)
    {
...
Retriever::got_word (const char *word, int location, int heading)
{
  ...
  if (trackWords && strlen (word) >= (unsigned int) minimumWordLength)
  {
    ...
    if (no_store_phrases)
    {
      ...
      {
        words_to_add.Add (w,
                          new word_entry (location, factor[heading],
                                          word_context));
      }
      else
      {
