...
public:
  ...
  {
    _persistent_connections = true;
  }
  void AvoidPersistentConnection ()
  {
    _persistent_connections = false;
  }
  ...
  unsigned int TcpWaitTime () const
  {
    return _tcp_wait_time;
  }
  unsigned int TcpMaxRetries () const
  {
    return _tcp_max_retries;
  }
  unsigned int MaxDocuments () const
  {
  ...
  const String & AcceptLanguage () const
  {
    return _accept_language;
  }
  bool DisableCookies () const
  {
    return _disable_cookies;
  }
  ...
  //
  int IsDisallowed (String url)
  {
    return _disallow.match (url, 0, 0);
  }
