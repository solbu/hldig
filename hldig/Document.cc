...
Transport::DocStatus Document::Retrieve (Server * server, HtDateTime date)
{
  ...
  else if (mystrncasecmp (url->service (), "https", 5) == 0)
  {
    ...
    if (HTTPSConnect)
    {
      ...
      if (server->IsPersistentConnectionAllowed ())
      {
        ...
        HTTPSConnect->DisablePersistentConnection ();
  ...
  if (transportConnect)
  {
    ...
    if (response)
    {
      ...
      if (transportConnect == HTTPConnect || transportConnect == HTTPSConnect
          || transportConnect == externalConnect)
...
int
Document::ShouldWeRetry (Transport::DocStatus DocumentStatus)
{
