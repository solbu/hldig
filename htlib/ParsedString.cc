...
//
#if RELEASE
static char RCSid[] = "$Id: ParsedString.cc,v 1.1.1.1 1997/02/03 17:11:05 turtle Exp $";
#endif
	...
	while (*str)
	{
		if (*str == '$')
		{
			...
			if (*str)
			{
				...
				{
					//
					// Error.  Probably an illegal value in the name
					// We'll assume the variable ended here.
					//
					temp = (ParsedString *) dict[variable];
					if (temp)
						parsed << temp->get(dict);
