...
protected:
  ...
  {
    if (n < 0 || n >= element_count)
    {
      fprintf (stderr, "HtVectorGType::CheckBounds: out of bounds.\n");
    }
  }
...
//      void  Assign(GType , int position);
  ...
  inline GType & operator[] (int n)
  {
    return Nth (n);
  }
  inline const GType & operator[] (int n) const
  {
    return Nth (n);
  }
  ...
  // Vector Assignment
  //
  HtVectorGType & operator= (HtVectorGType * vector)
  {
    return *this = *vector;
  }
...
public:
  ...
  inline void push_back (const GType & v)
  {
    Add (v);
  }
  ...
  inline const GType *begin () const
  {
    return (data);
  }
  inline GType *end ()
