...
insert_op2 (re_opcode_t op,
            unsigned char *loc, int arg1, int arg2, unsigned char *end)
{
  register unsigned char *pfrom = end;
    ...
    if (fastmap && startpos < total_size && !bufp->can_be_null)
    {
      ...
      else                      /* Searching backwards.  */
      {
        register char c = (size1 == 0 || startpos >= size1
                           ? string2[startpos - size1] : string1[startpos]);
...
re_match (struct re_pattern_buffer *bufp,
          const char *string, int size, int pos, struct re_registers *regs)
{
    ...
    case duplicate:
      {
        ...
        for (;;)
        {
          ...
          /* Compare that many; failure if mismatch, else move
             past them.  */
          if (translate
              ? bcmp_translate (d, d2, mcnt, translate) : bcmp (d, d2, mcnt))
            goto fail;
          d += mcnt, d2 += mcnt;
...
bcmp_translate (const char *s1,
                const char *s2, register int len, RE_TRANSLATE_TYPE translate)
{
