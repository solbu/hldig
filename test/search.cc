...
//
// $Id: search.cc,v 1.1.2.6 2000/09/21 04:25:34 ghutchis Exp $
//
...
WordTree *WordParser::ParseExpr(StringList& terms)
{
  ...
  if(!strcmp(term, "(")) {
    ...
    //
  } else if(!strcasecmp(term, "optional") ||
	    !strcasecmp(term, "or") ||
	    !strcasecmp(term, "and") ||
	    !strcasecmp(term, "near")) {
    expr = ParseConj(terms);
  } else if(!strcasecmp(term, "not") ||
	    !strcasecmp(term, "mandatory") ||
	    !strcasecmp(term, "forbiden")) {
...
{
  int op = 0;
  if(!strcasecmp(Term(terms), "mandatory"))
    op = WORD_TREE_MANDATORY;
  else if(!strcasecmp(Term(terms), "forbiden") ||
	  !strcasecmp(Term(terms), "not"))
    op = WORD_TREE_NOT;
...
{
  int op = 0;
  if(!strcasecmp(Term(terms), "optional"))
    op = WORD_TREE_OPTIONAL;
  else if(!strcasecmp(Term(terms), "or"))
    op = WORD_TREE_OR;
  else if(!strcasecmp(Term(terms), "and"))
    op = WORD_TREE_AND;
  else if(!strcasecmp(Term(terms), "near"))
    op = WORD_TREE_NEAR;
