...
//
int
WordListMulti::Open (const String & nfilename, int mode)
{
  filename = nfilename;
...
WordListMulti::Close ()
{
  if (isopen)
  {
    WordDBMulti *db;
    ListCursor cursor;
    for (dbs->Start_Get (cursor);
         (db = (WordDBMulti *) dbs->Get_Next (cursor));)
    {
      delete db->words;
    }
    dbs->Destroy ();
...
WordListMulti::Size () const
{
  ...
  if (isopen)
  {
         ...
         (db = (WordDBMulti *) dbs->Get_Next (cursor));)
    {
      if (!db->words->isopen)
      {
        if (db->words->Open (db->filename, O_RDONLY) != OK)
          return 0;
        size += db->words->Size ();
        if (db->words->Close () != OK)
          return 0;
      }
      else
      {
...
static int
merge_cmp_size (WordListMulti *, WordDBMulti * a, WordDBMulti * b)
{
  return b->size - a->size;
}

static int
merge_cmp_filename (WordListMulti *, WordDBMulti * a, WordDBMulti * b)
{
...
WordListMulti::Merge ()
{
  ...
  WordDBMulti *heap = new WordDBMulti[serial];
  {
    ...
    WordDBMulti *db;
    ListCursor cursor;
    for (i = 0, dbs->Start_Get (cursor);
         (db = (WordDBMulti *) dbs->Get_Next (cursor)); i++)
    {
      if (db->words->Open (db->filename, O_RDONLY) != OK)
        return NOTOK;
    ...
    dbs->Destroy ();
    myqsort ((void *) heap, serial, sizeof (WordDBMulti),
  ...
  while (serial > file_min)
  {
    ...
    if (a->words->Open (a->filename, O_RDONLY) != OK)
      return NOTOK;
    if (b->words->Open (b->filename, O_RDONLY) != OK)
      return NOTOK;
    if (tmp.Open (tmpname, O_RDWR) != OK)
      return NOTOK;
    if (tmp.db->CacheP () && tmp.db->CacheOff () != 0)
      return OK;
    ...
    while (reta == 0 && retb == 0)
    {
      ...
      {
        if (tmp.db->Put (0, keya, dataa, 0) != 0)
          return NOTOK;
        reta = cursora->Get (keya, dataa, DB_NEXT);
      }
      else
      {
        if (tmp.db->Put (0, keyb, datab, 0) != 0)
          return NOTOK;
        retb = cursorb->Get (keyb, datab, DB_NEXT);
      }
    }
    ...
    if (unlink ((char *) a->filename) != 0)
    {
      const String message =
        String ("WordListMulti::Merge: unlink ") + a->filename;
    ...
    {
      const String message =
        String ("WordListMulti::Merge: unlink ") + b->filename;
      perror ((const char *) message);
      return NOTOK;
    }
    if (use_compress)
    ...
    {
      const String message =
        String ("WordListMulti::Merge: rename ") + tmpname + String (" ") +
        b->filename;
      perror ((const char *) message);
      return NOTOK;
    }
    ...
    {
      if (rename
          ((char *) (tmpname + String ("_weakcmpr")),
           (char *) (b->filename + String ("_weakcmpr"))) != 0)
      {
        const String message =
          String ("WordListMulti::Merge: rename ") + tmpname +
          String ("_weakcmpr ") + b->filename + String ("_weakcmpr");
        perror ((const char *) message);
        return NOTOK;
      }
    }
  ...
  //
  myqsort ((void *) heap, serial, sizeof (WordDBMulti),
           (myqsort_cmp) merge_cmp_filename, (void *) this);
  int i;
  for (i = 0; i < serial; i++)
  {
    WordDBMulti *db = new WordDBMulti ();
    *db = heap[i];
    ...
    if (db->filename.compare (newname))
    {
      ...
      {
        const String message =
          String ("WordListMulti::Merge: rename ") + db->filename +
          String (" ") + newname;
        perror ((const char *) message);
        return NOTOK;
      }
      ...
      {
        if (rename
            ((char *) (db->filename + String ("_weakcmpr")),
             (char *) (newname + String ("_weakcmpr"))) != 0)
        {
          const String message =
            String ("WordListMulti::Merge: rename ") + db->filename +
            String ("_weakcmpr ") + newname + String ("_weakcmpr");
          perror ((const char *) message);
          return NOTOK;
        }
      }
...
//
int
WordListMulti::Override (const WordReference & arg)
{
  WordDBMulti *db = (WordDBMulti *) dbs->Last ();
  ...
  if (db->words->Size () > put_max)
  {
    ...
    if (db->words->Open (db->filename, db->mode) != OK)
      return NOTOK;
  }
...
// *****************************************************************************
int
WordListMulti::Exists (const WordReference &)
{
  return 0;
}
...
//
List *
WordListMulti::operator [] (const WordReference &)
{
  return 0;
#if 0
...
//
List *
WordListMulti::Prefix (const WordReference &)
{
  return 0;
#if 0
...
//
List *
WordListMulti::Collect (const WordReference &)
{
  return 0;
#if 0
...
//
int
WordListMulti::WalkDelete (const WordReference &)
{
  return 0;
#if 0
  DeleteWordData data;
...
int
WordListMulti::Delete (const WordReference &)
{
  return NOTOK;
}
...
//
int
WordListMulti::Noccurrence (const String &, unsigned int &) const
{
  return 0;
#if 0
  noccurrence = 0;
...
//
int
WordListMulti::Ref (const WordReference &)
{
  return NOTOK;
}
...
//
int
WordListMulti::Unref (const WordReference &)
{
  return NOTOK;
}
...
WordListMulti::AllRef ()
{
       ...
       (db = (WordDBMulti *) dbs->Get_Next (cursor));)
  {
    if (!db->words->isopen)
    {
      if (db->words->Open (db->filename, O_RDWR) != OK)
        return NOTOK;
      if (db->words->Close () != OK)
        return NOTOK;
    }
