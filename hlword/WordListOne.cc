...
static int
word_db_qcmp (WordContext * context, const WordDBCacheEntry * a,
              const WordDBCacheEntry * b)
{
  return WordKey::Compare (context, (const unsigned char *) a->key,
                           a->key_size, (const unsigned char *) b->key,
                           b->key_size);
}
...
//
int
WordListOne::Open (const String & nfilename, int mode)
{
  filename = nfilename;
  ...
  if (mode & O_TRUNC)
  {
    if (mode & O_RDWR)
    {
      ...
      fprintf (stderr,
               "WordListOne::Open: O_TRUNC | O_RDONLY is meaningless\n");
  }
  ...
  int ret =
    db->Open (filename, "index", DB_BTREE, flags, 0666,
              WORD_DB_INDEX) == 0 ? OK : NOTOK;
  if (ret == NOTOK)
    return ret;
  if (dict->Open () != OK)
    return NOTOK;
...
//
int
WordListOne::Override (const WordReference & arg)
{
  ...
  if (dict->SerialRef (word, wordid) != OK)
    return NOTOK;
  ...
  if (caches)
  {
    ...
    if (wordRef.Pack (key, record) != OK)
      return NOTOK;
    ret =
      caches->Add (key.get (), key.length (), record.get (),
                   record.length ()) == 0 ? OK : NOTOK;
    if (caches->Full ())
      caches->Merge (*db);
  }
  else
...
//
List *
WordListOne::operator [] (const WordReference & wordRef)
{
...
//
List *
WordListOne::Prefix (const WordReference & prefix)
{
  ...
  while (Dict ()->NextPrefix (cursor, word, record) == 0)
  {
    ...
    {
      WordReference *entry =
        (WordReference *) tmp_result->Shift (LIST_REMOVE_RELEASE);
      entry->SetWord (word);
      result->Push (entry);
    }
    delete tmp_result;
...
//
List *
WordListOne::Collect (const WordReference & wordRef)
{
  ...
  while (fgets (buffer, WORD_BUFFER_SIZE, f))
  {
    ...
    if (!line.empty ())
    {
      ...
      else
      {
        ...
        if (verbose && (inserted % 10000 == 0))
          fprintf (stderr, "WordList::Read: inserted %d entries\n", inserted);
        if (verbose > 1)
...
WordListOne::Write (FILE * f)
{
  ...
  while ((ret = dict->Next (cursor, word, wordinfo)) == 0)
  {
    ...
    data.word = word;
    WordCursor *search =
      Cursor (key, wordlist_walk_callback_file_out, (Object *) & data);
    search->Walk ();
    delete search;
  }
...
//
int
WordListOne::WalkDelete (const WordReference & wordRef)
{
  DeleteWordData data;
...
//
int
WordListOne::Noccurrence (const String & word, unsigned int &noccurrence) const
{
  return dict->Noccurrence (word, noccurrence);
}

WordKey
WordListOne::Key (const String & bufferin)
{
  WordKey key (context);
...
WordReference
WordListOne::Word (const String & bufferin, int exists /* = 1 */ )
{
  WordReference wordRef (context);
  StringList fields (bufferin, "\t ");
  String *field = (String *) fields.Get_First ();
