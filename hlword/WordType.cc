...

  {
    ...
    while (fl && fgets (buffer, sizeof (buffer), fl))
    {
      ...
      if (word && *word)
      {
        ...
        {
          fprintf (stderr,
                   "WordType::WordType: reading bad words from %s found %s, ignored because %s\n",
                   (const char *) filename, word,
                   (char *) NormalizeStatus (flags & WORD_NORMALIZE_NOTOK));
        }
        else
        {
...
WordType::Normalize (String & word) const
{
  ...
  //
  if (!alpha)
    return status | WORD_NORMALIZE_NOALPHA;
...
//
String WordType::NormalizeStatus (int flags)
{
  String
    tmp;

  if (flags & WORD_NORMALIZE_TOOLONG)
    tmp << "TOOLONG ";
  if (flags & WORD_NORMALIZE_TOOSHORT)
    tmp << "TOOSHORT ";
  if (flags & WORD_NORMALIZE_CAPITAL)
    tmp << "CAPITAL ";
  if (flags & WORD_NORMALIZE_NUMBER)
    tmp << "NUMBER ";
  if (flags & WORD_NORMALIZE_CONTROL)
    tmp << "CONTROL ";
  if (flags & WORD_NORMALIZE_BAD)
    tmp << "BAD ";
  if (flags & WORD_NORMALIZE_NULL)
    tmp << "NULL ";
  if (flags & WORD_NORMALIZE_PUNCTUATION)
    tmp << "PUNCTUATION ";
  if (flags & WORD_NORMALIZE_NOALPHA)
    tmp << "NOALPHA ";
...
//
String WordType::WordToken (const String tokens, int &current) const
{
  unsigned char
    text = tokens[current];
  String
