...
int
WordMeta::Initialize (WordList * nwords)
{
  words = nwords;
...
WordMeta::Open ()
{
  ...
  {
    if (CDB_memp_fget
        (imp->mpf, &imp->pgno, DB_MPOOL_NEW, (void **) &imp->info) != 0)
      return NOTOK;
...
int
WordMeta::Serial (int what, unsigned int &serial)
{
...
int
WordMeta::GetSerial (int what, unsigned int &serial)
{
  serial = WORD_META_SERIAL_INVALID;
  if (CDB_memp_fget (imp->mpf, &imp->pgno, 0, (void **) &imp->info) != 0)
    return NOTOK;
...
int
WordMeta::SetSerial (int what, unsigned int serial)
{
  if (CDB_memp_fget (imp->mpf, &imp->pgno, 0, (void **) &imp->info) != 0)
    return NOTOK;
...
int
WordMeta::Lock (const String & resource, WordLock * &lock)
{
  lock = new WordLock;
...
int
WordMeta::Unlock (const String & resource, WordLock * &lock)
{
  DB_ENV *dbenv = words->GetContext ()->GetDBInfo ().dbenv;
