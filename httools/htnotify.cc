...
//
// $Id: htnotify.cc,v 1.8 2004/05/28 13:15:25 lha Exp $
//
...
void add_notification (char *date, char *email, char *url,
                       const char *subject);
...
void
add_notification (char *date, char *email, char *url, const char *subject)
{
  ...
  EmailNotification *notif =
    new EmailNotification (date, email, url, subject);
  list->Add (notif);
}
...
//
void
send_notification (char *email, List * notifList)
{
...
//
void
send_email (List * notifList, String & command,
            String & to, String & listText, int singleSubject)
{
  ...
  String replyto = config->Find ("htnotify_replyto");
