...
//
Dictionary *
purgeDocs (Dictionary * purgeURLs)
{
  HtConfiguration *config = HtConfiguration::config ();
  ...
  while ((id = (IntObject *) IDs->Get_Next ()))
  {
    ...
    remove_unused =
      config->Boolean ("server", u_url.host (), "remove_bad_urls");
    remove_unretrieved =
      config->Boolean ("server", u_url.host (), "remove_unretrieved_urls");
    idStr = 0;
    idStr << id->Value ();
...
{
public:
  DeleteWordData (const Dictionary & discard_arg):discard (discard_arg)
  {
    deleted = remains = 0;
  }
...
delete_word (WordList * words, WordDBCursor & cursor,
             const WordReference * word_arg, Object & data)
{
...
//
void
purgeWords (Dictionary * discard_list)
{
  HtConfiguration *config = HtConfiguration::config ();
