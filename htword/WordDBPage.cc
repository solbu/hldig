...
static const char *version_label[] =
  { "INVALID_VERSION_0", "INVALID_VERSION_1", "INVALID_VERSION_2",
"14 Dec 1999", "3 Jan 2000", NULL };
...
get_version_label (int v)
{
  ...
  if (COMPRESS_VERSION < 0
      || COMPRESS_VERSION >
      ((sizeof (version_label) / sizeof (*version_label)) - 1))
  {
    errr
      ("get_version_label: version_label[COMPRESS_VERSION] is not valid, please update version_label");
  }
...
WordDBPage::TestCompress (int debuglevel)
{
  if (debuglevel > 2)
  {
    printf ("ttttttttttttt WordDBPage::TestCompress  BEGIN\n");
  }
  ...
  {
    errr ("WordDBPage::TestCompress: Compress failed");
  }

  if (debuglevel > 2)
  {
    printf ("ttttttttttttt WordDBPage::TestCompress  END\n");
  }
  return OK;
}
...
static int
first_diff (const String & s1, const String & s2)
{
  int j;
                        ...
                        DB_CMPR_INFO * /*=NULL*/ )
{
  ...
  if (verbose)
  {
    printf ("uuuuuuuuu WordDBPage::Uncompress: BEGIN\n");
  }


  // ** first check if versions are OK
  int read_version =
    pin->get_uint (NBITS_COMPRESS_VERSION, "COMPRESS_VERSION");
  if (read_version != COMPRESS_VERSION)
  {
    fprintf (stderr,
             "WordDBPage::Uncompress: ***        Compression version mismatch      ***\n");
    ...
    fprintf (stderr,
             "Are you sure you're not reading an old DB with a newer version of the indexer??\n");
    errr
      ("WordDBPage::Uncompress: ***        Compression version mismatch      ***");
    exit (1);
  }
  ...
  if (verbose)
  {
    printf ("uuuuuuuuu WordDBPage::Uncompress: END\n");
  }
  return OK;
}
...
WordDBPage::Uncompress_main (Compressor * pin)
{
  if (!pin)
  {
    errr ("WordDBPage::Uncompress: no Compressor to uncompress from!!");
  }
  ...
  if (nkeysleft > 0)
  {
    ...
    if (type == P_LBTREE)
    {
      uncompress_data (in, 0, key0.RecType ());
    }
    nkeysleft--;
  }
  if (nkeysleft > 0 && type == P_IBTREE)
  {
    uncompress_key (in, 1);
    nkeysleft--;
  }
  ...
  if (nkeysleft > 0)
  {
    ...
    {
      if (verbose)
        printf ("field %2d : start position:%4d  \n", j, in.size ());
      if (j == 3 && verbose)
      {
        in.verbose = 2;
      }
      ...
      if (verbose)
      {
        printf ("WordDBPage::Uncompress_main:got numfield:%2d:nvals:%4d\n", j,
                rnum_sizes[j]);
      }
    }
    ...
    Uncompress_show_rebuild (rnums, rnum_sizes, nnums, rworddiffs,
                             nrworddiffs);
...
void
WordDBPage::Uncompress_vals_chaged_flags (Compressor & in,
                                          unsigned int **pcflags, int *pn)
{
...
WordDBPage::Uncompress_header (Compressor & in)
{
  ...
  pg->lsn.offset =
    in.get_uint_vl (8 * sizeof (pg->lsn.offset), "page:lsn.offset");
  pg->pgno = in.get_uint_vl (8 * sizeof (pg->pgno), "page:pgno");
  pg->prev_pgno =
    in.get_uint_vl (8 * sizeof (pg->prev_pgno), "page:prev_pgno");
  pg->next_pgno =
    in.get_uint_vl (8 * sizeof (pg->next_pgno), "page:next_pgno");
  pg->entries = in.get_uint_vl (8 * sizeof (pg->entries), "page:entries");
  pg->hf_offset =
    in.get_uint_vl (8 * sizeof (pg->hf_offset), "page:hf_offset");
                                ...
                                int nrworddiffs)
{
  ...
  {
    i0 = 1;
  }                             // internal pages have particular first key
  ...
  for (ii = i0; ii < nk; ii++)
  {
    ...
    {
      // **** get the data fields
      arec.set_decompress (rnums, rnum_sizes, ii, CNDATADATA, CNDATASTATS0,
                           CNDATASTATS1);
    }
    else
    {
      if (type != 3)
      {
        errr ("WordDBPage::Uncompress_rebuild: unsupported type!=3");
      }
    ...
    if (ii > i0)
    {
      ...
      {
        foundfchange = 1;
        if (rnum_pos[CNWORDDIFFLEN] >= rnum_sizes[CNWORDDIFFLEN])
        {
          errr ("WordDBPage::Uncompress read wrong num worddiffs");
        }
      ...
      for (j = 1; j < nfields; j++)
      {
        ...
        if (changed)
        {
          ...
          int indx = rnum_pos[k];
          if (indx >= rnum_sizes[k])
          {
            errr
              ("WordDBPage::Uncompress read wrong num of changes in a field");
          }
          if (!foundfchange)
          {
                                     ...
                                     int nrworddiffs)
{
  ...
  {
    printf
      ("WordDBPage::Uncompress_show_rebuild: rebuilt numerical fields\n");
    for (j = 0; j < nnums0; j++)
    {
    ...
    printf ("reswordiffs:");
    for (i = 0; i < nrworddiffs; i++)
    {
      printf ("%c", (isalnum (rworddiffs[i]) ? rworddiffs[i] : '#'));
    }
    printf ("\n");
  }
}
...
WordDBPage::Compress (int ndebug, DB_CMPR_INFO * cmprInfo /*=NULL*/ )
{
  ...
  res->put_uint (COMPRESS_VERSION, NBITS_COMPRESS_VERSION,
                 "COMPRESS_VERSION");
  res->put_uint (CMPRTYPE_NORMALCOMRPESS, NBITS_CMPRTYPE, "CMPRTYPE");

  if (verbose)
  {
    printf ("WordDBPage::Compress: trying normal compress\n");
  }
  ...
  if (cmpr_ok != OK || res->buffsize () > pgsz)
  {
    ...
    {
      printf
        ("WordDBCompress::Compress full compress failed ... not compressing at all\n");
    }
    show ();
...
WordDBPage::Compress_main (Compressor & out)
{
  ...
  if (verbose)
  {
    printf ("WordDBPage::Compress_main: starting compression\n");
  }
...
//bmt_START;
  ...
  if (nk > 0)
  {
    ...
    if (verbose)
      Compress_show_extracted (nums, nums_pos, nnums, worddiffs);
  }
...
//bmt_END;bmt_START;
    ...
    // compress worddiffs
    int size =
      out.put_fixedbitl (worddiffs.begin (), worddiffs.size (), "WordDiffs");
    if (verbose)
      printf ("compressed wordiffs : %3d values: %4d bits %4f bytes\n",
...
void
WordDBPage::Compress_extract_vals_wordiffs (int *nums, int *nums_pos, int,
                                            HtVector_byte & worddiffs)
{
  WordDBKey pkey;
  ...
  for (ii = i0; ii < nk; ii++)
  {
    ...
    if (type == P_LBTREE)
    {
      ...
      if (arec.type == WORD_RECORD_STATS)
      {
        nums[CNDATASTATS0 * nk + nums_pos[CNDATASTATS0]++] =
          arec.info.stats.noccurrence;
        nums[CNDATASTATS1 * nk + nums_pos[CNDATASTATS1]++] =
          arec.info.stats.ndoc;
      }
      else if (arec.type == WORD_RECORD_DATA)
      {
    ...
    else
    {
      ...
      {
        errr
          ("WordDBPage::Compress_extract_vals_wordiffs: unsupported type!=3");
      }
      // ****** btree internal page specific
    ...
    if (ii > i0)
    {
      ...
      if (!(aword == pword))
      {
        ...
        nums[CNWORDDIFFLEN * nk + nums_pos[CNWORDDIFFLEN]++] =
          aword.length () - fd;
        for (int s = fd; s < aword.length (); s++)
        {
          worddiffs.push_back (aword[s]);
        }
      }
    }
    pkey = akey;
...
void
WordDBPage::Compress_vals_changed_flags (Compressor & out,
                                         unsigned int *cflags, int n)
{
  int size = out.size ();
  ...
  if (verbose)
    printf
      ("compressed flags %2d : %3d values: %4d bits %8f bytes  : ended bit field pos:%6d\n",
       0, n, size, size / 8.0, out.size ());

}

void
WordDBPage::Compress_vals (Compressor & out, int *nums, int *nums_pos,
                           int nnums0)
{
  // the changed flags fields are particular
  Compress_vals_changed_flags (out, (unsigned int *) (nums + 0 * nk),
                               nums_pos[0]);
  ...
  for (int j = 1; j < nnums0; j++)
  {
    ...
    if (verbose)
      printf
        ("compressed field %2d : %3d values: %4d bits %8f bytes  : ended bit field pos:%6d\n",
...
// no smart compression ... for now
  ...
  out.put_uint_vl (pg->lsn.offset, 8 * sizeof (pg->lsn.offset),
                   "page:lsn.offset");
  out.put_uint_vl (pg->pgno, 8 * sizeof (pg->pgno), "page:pgno");
  out.put_uint_vl (pg->prev_pgno, 8 * sizeof (pg->prev_pgno),
                   "page:prev_pgno");
  out.put_uint_vl (pg->next_pgno, 8 * sizeof (pg->next_pgno),
                   "page:next_pgno");
  out.put_uint_vl (pg->entries, 8 * sizeof (pg->entries), "page:entries");
  out.put_uint_vl (pg->hf_offset, 8 * sizeof (pg->hf_offset),
                   "page:hf_offset");
...
void
WordDBPage::Compress_show_extracted (int *nums, int *nums_pos, int nnums0,
                                     HtVector_byte & worddiffs)
{
  int i, j;
  ...
  for (i = 0; i < mx; i++)
  {
    ...
    if (w < worddiffs.size ())
    {
      printf ("   %02x %c ", worddiffs[w],
              (isalnum (worddiffs[w]) ? worddiffs[w] : '#'));
    }
    w++;
    printf ("\n");
  }
...
WordDBPage::Compare (WordDBPage & other)
{
  ...
  {
    res++;
    printf ("compare failed for  pgsz                 \n");
  }
  if (other.pg->lsn.file != pg->lsn.file)
  {
    res++;
    printf ("compare failed for  pg->lsn.file         \n");
  }
  if (other.pg->lsn.offset != pg->lsn.offset)
  {
    res++;
    printf ("compare failed for  pg->lsn.offset       \n");
  }
  if (other.pg->pgno != pg->pgno)
  {
    res++;
    printf ("compare failed for  pg->pgno             \n");
  }
  if (other.pg->prev_pgno != pg->prev_pgno)
  {
    res++;
    printf ("compare failed for  pg->prev_pgno        \n");
  }
  if (other.pg->next_pgno != pg->next_pgno)
  {
    res++;
    printf ("compare failed for  pg->next_pgno        \n");
  }
  if (other.pg->entries != pg->entries)
  {
    res++;
    printf ("compare failed for  pg->entries          \n");
  }
  if (other.pg->hf_offset != pg->hf_offset)
  {
    res++;
    printf ("compare failed for  pg->hf_offset        \n");
  }
  if (other.pg->level != pg->level)
  {
    res++;
    printf ("compare failed for  pg->level            \n");
  }
  if (other.pg->type != pg->type)
  {
    res++;
    printf ("compare failed for  pg->type             \n");
  }
  int i, k;
  // double check header
  if (memcmp
      ((void *) pg, (void *) other.pg, sizeof (PAGE) - sizeof (db_indx_t)))
  {
    res++;
  ...
  for (i = 0; i < (type == P_LBTREE ? pg->entries / 2 : pg->entries); i++)
  {
    ...
    else
    {
      if (type != 3)
      {
        errr ("WordDBPage::Compare: unsupported type!=3");
      }
          ...
          btikey (i)->nrecs != other.btikey (i)->nrecs)
      {
        ...
        printf ("this :len   :%4d type  :%4d pgno  :%4d nrecs :%4d \n",
                btikey (i)->len, btikey (i)->type, btikey (i)->pgno,
                btikey (i)->nrecs);
  ...
  if (pg->entries > 0)
  {
    ...
    int other_smallestoffset =
      HtMaxMin::min_v (other.pg->inp, other.pg->entries);
...
WordDBPage::show ()
{
  ...
  if (pg->type == 5)
  {
    ...
    for (i = 0; i < pg->entries; i++)
    {
      if ((i % 2) && dud.type == WORD_RECORD_NONE)
      {
        continue;
      }
      printf ("\n||%c:%3d:off:%03d:invoff:%4d:len:%2d:typ:%x:",
              i % 2 ? 'D' : 'K', i, e_offset (i), pgsz - e_offset (i),
              entry (i)->len, entry (i)->type);
      if (i > 0)
      {
        l = entry (i)->len + 3;
      ...
      if (!(i % 2))
      {
        ...
        printf ("\"");
        for (j = 0; j < 20 - tkey.GetWord ().length (); j++)
        {
          printf (" ");
        }
        printf ("|");
        ...
        for (j = 1; j < tkey.NFields (); j++)
        {
          if (fieldchanged[j])
          {
            keycl += WordKeyInfo::Instance ()->sort[j].bits;
          }
        }
        if (fieldchanged[0])
        {
          keycl += 3;
          keycl += 8 * strlen (wordchange);
        }
      ...
      else
      {
        if (entry (i)->len > 100)
        {
          printf ("WordDBPage::show: aaargh strange failing\n");
          return;
        }
  ...
  if (pg->type == 3)
  {
    for (i = 0; i < pg->entries; i++)
    {
      ...
      printf ("%3d: off:%4d:len:%3d :type:%3d :pgno:%4d: nrecs:%4d:: ", i,
              pg->inp[i], bie->len, bie->type, bie->pgno, bie->nrecs);
      WordDBKey tkey (bie);
      for (j = 0; j < bie->len - tkey.GetWord ().length (); j++)
      {
        printf ("%2x ", bie->data[j]);
      }
      printf (" : ");
      for (j = 1; j < tkey.NFields (); j++)
      {
        printf ("%5d ", tkey.Get (j));
      }
      printf ("\"%s\"\n", (char *) tkey.GetWord ());
    }
  }
