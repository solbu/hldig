...
int
WordDict::Initialize (WordList * nwords)
{
  words = nwords;
...
WordDict::Open ()
{
  ...
  return db->Open (filename, "dict", DB_BTREE, flags, 0666,
                   WORD_DB_DICT) == 0 ? OK : NOTOK;
}
...
int
WordDict::Serial (const String & word, unsigned int &serial)
{
  int ret;
...
int
WordDict::SerialExists (const String & word, unsigned int &serial)
{
  int ret;
  WordDictRecord entry;
  if ((ret = entry.Get (db, word)) != 0 && ret != DB_NOTFOUND)
...
int
WordDict::SerialRef (const String & word, unsigned int &serial)
{
  int ret;
  WordDictRecord entry;
  if ((ret = entry.Get (db, word)) != 0 && ret != DB_NOTFOUND)
...
int
WordDict::Noccurrence (const String & word, unsigned int &noccurrence) const
{
  if (word.empty ())
  {
...
int
WordDict::Normalize (String & word) const
{
...
int
WordDict::Incr (const String & word, unsigned int incr)
{
  int ret;
  WordDictRecord entry;
  if ((ret = entry.Get (db, word)) != 0 && ret != DB_NOTFOUND)
...
int
WordDict::Decr (const String & word, unsigned int decr)
{
  WordDictRecord entry;
  int ret;
  if ((ret = entry.Get (db, word)) != 0)
  {
    if (ret == DB_NOTFOUND)
      fprintf (stderr,
               "WordDict::Unref(%s) Unref on non existing word occurrence\n",
               (const char *) word);
    return NOTOK;
  }
  entry.count -= decr;
...
int
WordDict::Put (const String & word, unsigned int noccurrence)
{
  int ret;
  WordDictRecord entry;
  if ((ret = entry.Get (db, word)) != 0 && ret != DB_NOTFOUND)
...
int
WordDict::Exists (const String & word) const
{
...
int
WordDict::Next (WordDictCursor * cursor, String & word,
...
WordDictCursor *
WordDict::CursorPrefix (const String & prefix) const
{
  WordDictCursor *cursor = new WordDictCursor;
  cursor->cursor = db->Cursor ();
...
int
WordDict::NextPrefix (WordDictCursor * cursor, String & word,
                      WordDictRecord & record)
{
  String coded;
  int ret;
