...
public:
  ...
  inline ~ WordDBCache ()
  {
    ...
    {
      fprintf (stderr,
               "WordDBCache::~WordDBCache: destructor called and cache not empty\n");
    }
    free (entries);
    free (pool);
  }
  ...
  inline int ResizeEntries ()
  {
    entries_size *= 2;
    entries =
      (WordDBCacheEntry *) realloc (entries,
                                    entries_size * sizeof (WordDBCacheEntry));
  ...
  inline int
    SetCompare (int (*ncompare)
                (WordContext *, const WordDBCacheEntry *,
                 const WordDBCacheEntry *))
  {
    compare = ncompare;
    return 0;
  }
  ...
  inline int Sort ()
  {
    ...
    //
    myqsort ((void *) entries, entries_length, sizeof (WordDBCacheEntry),
             (myqsort_cmp) compare, (void *) context);
    return 0;
  }
...
private:
  ...
  int (*compare) (WordContext *, const WordDBCacheEntry *,
                  const WordDBCacheEntry *);
  WordContext *context;
};
...
{
public:
  inline WordDBCaches (WordList * nwords, int nfile_max, int size_hint,
                       int nsize_max):cache (nwords->GetContext ())
  {
    words = nwords;

    files = new WordDB (words->GetContext ()->GetDBInfo ());
    files->Open (words->Filename (), "tmp", DB_BTREE, words->Flags (), 0666,
                 WORD_DB_FILES);
    file_max = nfile_max;
    size_max = nsize_max;
    lock = 0;
  ...
  int Full () const
  {
    return size_max > 0 ? size >= size_max : 0;
  }
  ...
  int Merge ();
  int Merge (const String & filea, const String & fileb,
             const String & tmpname);
  int Merge (WordDB & db);

  int CacheWrite (const String & filename);
  int
    CacheCompare (int (*compare)
                  (WordContext *, const WordDBCacheEntry *,
                   const WordDBCacheEntry *))
  {
    cache.SetCompare (compare);
    return OK;
  }

  int WriteEntry (FILE * fp, WordDBCacheEntry & entry, unsigned char *&buffer,
                  unsigned int &buffer_size);
  int ReadEntry (FILE * fp, WordDBCacheEntry & entry, unsigned char *&buffer,
                 unsigned int &buffer_size);
