...
int
htdig_index_open (htdig_parameters_struct * htdig_parms)
{
  int ret = -1;
  ...
  if (strlen (htdig_parms->meta_description_factor) > 0)
  {
    config->Add ("meta_description_factor",
                 htdig_parms->meta_description_factor);
  }
  ...
  {
    reportError (form
                 ("[HTDIG] Invalid url_part_aliases or common_url_parts: %s",
                  url_part_errors.get ()));
 ...
 *******************************************************/
int
htdig_index_simple_doc (htdig_simple_doc_struct * a_simple_doc)
{
  int index_error = 0;
  //int ret = 0;
...
htdig_index_close (void)
{
  ...
  if (htdig_index_open_flag == TRUE)
  {
    ...
    if (debug != 0)
    {
      ...
      {
        reportError (form
                     ("[HTDIG] Error closing log file . Error:[%d], %s\n",
...
int
htdig_index_test_url (htdig_parameters_struct * htdig_parms)
{
  //int ret = FALSE;
  ...
  cout << "   valid_extensions = " << config->
    Find ("valid_extensions") << endl;
  ...
  cout << "   limit_normalized = " << config->
    Find ("limit_normalized") << endl;
