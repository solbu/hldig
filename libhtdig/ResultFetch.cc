...
ResultFetch::fetchMatch (ResultMatch * match, DocumentRef * ref, int current)
{
  ...
  vars.Add ("SIZEK",
            new String (form ("%d", (ref->DocSize () + 1023) / 1024)));

  if (maxScore != 0 && maxScore != minScore)
  {
    int percent =
      (int) ((ref->DocScore () - minScore) * 100 / (maxScore - minScore));
    if (percent <= 0)
      percent = 1;
...
ResultFetch::generateStars (DocumentRef * ref, int right)
{
  ...
  if (maxScore != 0 && maxScore != minScore)
  {
    ...
    if (debug)
      cerr << "generateStars: doc, min, max " << ref->DocScore () << ", " <<
...
ResultFetch::buildMatchList ()
{
  ...
  int dategiven = ((config->Value ("startmonth")) ||
                   ...
                   (config->Value ("endmonth")) || (config->Value ("endday"))
...
//
          ...
          (Collection *) selected_collections->Get_NextElement ()))
  {
    ...
    while ((cpid = results->Get_Next ()))
    {
      ...
      {
        date_score =
          date_factor * ((thisRef->DocTime () * 1000.0 / (double) now) - 900);
        score += date_score;
      }
      ...
      if (backlink_factor != 0.0)
      {
        ...
        backlink_score =
          backlink_factor * (thisRef->DocBackLinks () / (double) links);
      ...
      if (debug)
      {
        ...
        cerr << "   score " << score << "(" << thisMatch->getScore () <<
          "), maxScore " << maxScore << ", minScore " << minScore << endl;
      }
  ...
  for (matches_part = matches.Get_First (); matches_part != 0;
...
//*****************************************************************************
String *
ResultFetch::excerpt (ResultMatch * match, DocumentRef * ref,
...
//*****************************************************************************
String
ResultFetch::hilight (ResultMatch * match, const String & str_arg,
...
//TODO: Look into using native windows system logs instead
  ...
  syslog (level, "%s [%s] (%s) [%s] [%s] (%d/%s) - %d -- %s\n",
          ...
          (const char *) config->Find ("match_method"), input->get ("words"),
          logicalWords.get (), nMatches,
          (const char *) config->Find ("matches_per_page"), page, ref);
#endif
}
