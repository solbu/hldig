...
HtFTP::DocStatus HtFTP::Request ()
{
  ...
  {
    String *
      mime_type = (String *) mime_map->Find (ext + 1);
    if (mime_type)
  ...
  {
    if ((mystrcasecmp (ext, ".html") == 0)
        || (mystrcasecmp (ext, ".htm") == 0))
      _response._content_type = "text/html";
  ...
  FILE *
    f = fopen ((const char *) _url.path (), "r");
  if (f == NULL)
