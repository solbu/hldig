...
const String *
HtFile::Ext2Mime (const char *ext)
{
  ...
  if (!mime_map)
  {
    ...
    if (in)
    {
      ...
      while (in >> line)
      {
        ...
        {
          if (debug > 3)
            cout << "MIME: " << split_line[i] << "\t-> " << mime_type << endl;
...
String
HtFile::File2Mime (const char *fname)
{
  HtConfiguration *config = HtConfiguration::config ();
...
HtFile::DocStatus HtFile::Request ()
{
            ...
            && lstat (filename.get (), &stat_buf) == 0)
        {
          ...
          for (i = 0; (stat_buf.st_mode & S_IFMT) == S_IFLNK && i < 10; i++)
          {
            char
              link[100];
            int
              count = readlink (filename.get (), link, sizeof (link) - 1);
            ...
            URL
            newURL (encodedName, _url); // resolve relative paths
  ...
  if (_modification_time
      && *_modification_time >= HtDateTime (stat_buf.st_mtime))
    return Transport::Document_not_changed;

  bool
    unknown_ext = false;
  char *
    ext = strrchr (path.get (), '.');
  if (ext == NULL)
    unknown_ext = true;
  else
  {
    const String *
      mime_type = Ext2Mime (ext + 1);
    if (mime_type)
