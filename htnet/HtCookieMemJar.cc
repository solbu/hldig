...
// Constructor
HtCookieMemJar::HtCookieMemJar ():_key (0), _list (0), _idx (0)
{
...
// Copy constructor
HtCookieMemJar::HtCookieMemJar (const HtCookieMemJar & rhs):_key (0), _list (0),
_idx (0)
{
  ...
  if (rhs.cookieDict)
  {
    ...
    while (char *d = rhs.cookieDict->Get_Next ())
    {
      ...
      if (List * rhsl = (List *) rhs.cookieDict->Find (d))
      {
        ...
        {
          HtCookie *
            new_cookie = new HtCookie (*cookie);
...
int
HtCookieMemJar::AddCookie (const String & CookieString, const URL & url)
{
...
int
HtCookieMemJar::AddCookieForHost (HtCookie * cookie, String HostName)
{
...
/////////////////////////////////////////////////////////////   
  ...
  else
  {
    ...
    else
    {
      ...
      if (num_periods >= minimum_periods)       // here is a so-far valid domain
      {
        ...
        {
          if (debug > 2)
            cout << "Cookie - valid domain: " << Domain << endl;
        }
        else if (HostName.length () == 0)
        {
          if (debug > 2)
            cout << "Imported cookie - valid domain: " << Domain << endl;
        }
        else
        {
          cookie->SetIsDomainValid (false);
  ...
  if (cookieDict->Exists (Domain) == 0)
  {
    ...
    list = (List *) cookieDict->Find (Domain);
...
List *
HtCookieMemJar::cookiesForDomain (const String & DomainName)
{
  List *list;
...
int
HtCookieMemJar::SetHTTPRequest_CookiesString (const URL & _url,
                                              String & RequestString)
{
  ...
  if (debug > 3)
    cout << "Looking for cookies - Domain: "
      << Domain << " (Minimum periods: " << minimum_periods << ")" << endl;
  ...
  if (num_periods >= minimum_periods && cookieDict->Exists (Domain))
...
int
HtCookieMemJar::WriteDomainCookiesString (const URL & _url,
                                          const String & Domain,
                                          String & RequestString)
{
  ...
  if (cookieList)
  {
    ...
    while ((cookie = (HtCookie *) cookieList->Get_Next ()))
    {
      ...
      //
      const bool expired = (cookie->GetExpires () && (*(cookie->GetExpires ()) < now))  // Expires
        || (HtDateTime::GetDiff (now, cookie->GetIssueTime ()) <= cookie->GetMaxAge ());        // Max-age
...
ostream & HtCookieMemJar::ShowSummary (ostream & out)
{
  ...
  int
    num_server = 0;             // Number of servers with cookies
