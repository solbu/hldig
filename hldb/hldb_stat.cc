...
typedef enum
{ T_NOTSET, T_DB, T_ENV, T_LOCK, T_LOG, T_MPOOL, T_TXN } test_t;
...
main (int argc, char *argv[])
{
  ...
  case T_DB:
    ...
    if ((ret =
         dbp->open (dbp, db, subdb, DB_UNKNOWN, (DB_RDONLY | compress),
                    0)) != 0)
    {
...
btree_stats (DB_ENV * dbenvp, DB * dbp)
{
  ...
  dl ("Number of bytes free in tree leaf pages", (u_long) sp->bt_leaf_pgfree);
  printf (" (%.0f%% ff).\n",
...
hash_stats (DB_ENV * dbenvp, DB * dbp)
{
  ...
  dl ("Number of bytes free in overflow pages", (u_long) sp->hash_big_bfree);
  printf (" (%.0f%% ff).\n",
  ...
  dl ("Number of bytes free in duplicate pages", (u_long) sp->hash_dup_free);
  printf (" (%.0f%% ff).\n",
...
queue_stats (DB_ENV * dbenvp, DB * dbp)
{
  ...
  printf ("%lu\tLast allocated record number.\n", (u_long) sp->qs_cur_recno);
...
mpool_stats (DB_ENV * dbenvp)
{
  ...
  for (; fsp != NULL && *fsp != NULL; ++fsp)
  {
    ...
    dl ("Requested pages found in the cache", (u_long) (*fsp)->st_cache_hit);
