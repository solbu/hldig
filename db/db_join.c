...
#ifndef lint
static const char revid[] = "$Id: db_join.c,v 1.1.2.3 2000/09/17 01:35:04 ghutchis Exp $";
#endif /* not lint */
	...
	u_int32_t flags;
{
	...
	if ((ret = CDB___os_calloc(dbenv, 1, sizeof(DBC), &dbc)) != 0)
		goto err;
	...
	if ((ret = CDB___os_malloc(dbenv, 256, NULL, &jc->j_key.data)) != 0)
		goto err;
	jc->j_key.ulen = 256;
	...
	if ((ret = CDB___os_calloc(dbenv, nslots, sizeof(DBC *),
	    &jc->j_curslist)) != 0)
		goto err;
	if ((ret = CDB___os_calloc(dbenv, nslots, sizeof(DBC *),
	    &jc->j_workcurs)) != 0)
	...
	if ((ret = CDB___os_calloc(dbenv, nslots, sizeof(DBC *),
	    &jc->j_fdupcurs)) != 0)
		goto err;
	if ((ret = CDB___os_calloc(dbenv, nslots, sizeof(u_int8_t),
	    &jc->j_exhausted)) != 0)
		goto err;
	...
	u_int32_t flags;
{
	...
	if ((ret = CDB___db_joingetchk(dbp, key_arg, flags)) != 0)
		return (ret);
...
retry2:		cp = jc->j_workcurs[i];
		...
		if (ret == ENOMEM) {
			jc->j_key.ulen <<= 1;
			if ((ret = CDB___os_realloc(dbp->dbenv, jc->j_key.ulen,
			    NULL, &jc->j_key.data)) != 0) {
mem_err:			CDB___db_err(dbp->dbenv,
	...
	DBC *dbc;
{
	 ...
	 * CDB___db_close may loop indefinitely.
	 */
	...
	CDB___os_free(jc->j_workcurs, 0);
	CDB___os_free(jc->j_fdupcurs, 0);
	...
	u_int32_t exhausted;
{
	...
	case 0:
		...
		if (cmp == 0) {
			 ...
			 * above.
			 */
			if ((ret = CDB___db_retcopy(dbp, data, ldata.data,
			    ldata.size, &data->data, &data->size)) != 0)
				return (ret);
			CDB___os_free(ldata.data, 0);
			return (0);
		}
