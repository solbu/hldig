...
int
CDB___bam_pg_alloc_log (dbenv, txnid, ret_lsnp, flags,
     ...
     db_pgno_t next;
{
  ...
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    + sizeof (fileid)
    + sizeof (*meta_lsn)
    + sizeof (*page_lsn) + sizeof (pgno) + sizeof (ptype) + sizeof (next);
...
int
CDB___bam_pg_free_log (dbenv, txnid, ret_lsnp, flags,
...
int
CDB___bam_split_log (dbenv, txnid, ret_lsnp, flags,
                     fileid, left, llsn, right, rlsn, indx, npgno, nlsn, pg)
     DB_ENV *dbenv;
     DB_TXN *txnid;
     DB_LSN *ret_lsnp;
     u_int32_t flags;
...
int
CDB___bam_rsplit_log (dbenv, txnid, ret_lsnp, flags,
...
int
CDB___bam_adj_log (dbenv, txnid, ret_lsnp, flags,
     ...
     u_int32_t is_insert;
{
  ...
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    + sizeof (fileid)
    + sizeof (pgno)
    + sizeof (*lsn) + sizeof (indx) + sizeof (indx_copy) + sizeof (is_insert);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
...
int
CDB___bam_cadjust_log (dbenv, txnid, ret_lsnp, flags,
     ...
     int32_t total;
{
  ...
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    + sizeof (fileid)
    + sizeof (pgno)
    + sizeof (*lsn) + sizeof (indx) + sizeof (adjust) + sizeof (total);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
...
int
CDB___bam_cdel_log (dbenv, txnid, ret_lsnp, flags, fileid, pgno, lsn, indx)
     DB_ENV *dbenv;
     DB_TXN *txnid;
     DB_LSN *ret_lsnp;
     u_int32_t flags;
     ...
     u_int32_t indx;
{
  ...
  if (txnid == NULL)
  {
    ...
    lsnp = &txnid->last_lsn;
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    + sizeof (fileid) + sizeof (pgno) + sizeof (*lsn) + sizeof (indx);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
...
int
CDB___bam_repl_log (dbenv, txnid, ret_lsnp, flags,
...
int
CDB___bam_root_log (dbenv, txnid, ret_lsnp, flags,
     ...
     DB_LSN *meta_lsn;
{
  ...
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    + sizeof (fileid)
    + sizeof (meta_pgno) + sizeof (root_pgno) + sizeof (*meta_lsn);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
     ...
     DB_ENV *dbenv;
{
  ...
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___bam_pg_alloc_print,
                                    DB_bam_pg_alloc)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___bam_pg_free_print,
                                    DB_bam_pg_free)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___bam_split_print,
                                    DB_bam_split)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___bam_rsplit_print,
                                    DB_bam_rsplit)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
