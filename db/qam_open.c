...
#ifndef lint
static const char revid[] = "$Id: qam_open.c,v 1.1.2.2 2000/09/14 03:13:22 ghutchis Exp $";
#endif /* not lint */
 ...
 * CDB___qam_open
 *
 * PUBLIC: int CDB___qam_open __P((DB *, const char *, db_pgno_t, u_int32_t));
 */
int
CDB___qam_open(dbp, name, base_pgno, flags)
	DB *dbp;
	const char *name;
	db_pgno_t base_pgno;
	u_int32_t flags;
...
again:	if (qmeta->dbmeta.magic == DB_QAMMAGIC) {
	...
	if ((ret = CDB_memp_fsync(dbp->mpf)) == DB_INCOMPLETE) {
		CDB___db_err(dbp->dbenv, "Flush of metapage failed");
		ret = EINVAL;
	}
