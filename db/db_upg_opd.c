...
static const char revid[] =
  "$Id: db_upg_opd.c,v 1.2 2002/02/02 18:18:05 ghutchis Exp $";
#endif /* not lint */
     ...
     db_pgno_t *pgnop;
{
  ...
  if (cur_cnt > 1)
  {
    ...
    if ((ret = CDB___os_malloc (dbp->dbenv, dbp->pgsize, NULL, &ipage)) != 0)
      goto err;
  ...
  for (level = LEAFLEVEL + 1; cur_cnt > 1; ++level)
  {
    for (indx = 0, i = next_cnt = 0; i < cur_cnt;)
    {
      ...
      if (sorted)
      {
        ...
        if ((ret = __db_build_ri (dbp, fhp, ipage, page, indx, &nomem)) != 0)
        goto err;
      if (nomem)
      {
        indx = 0;
     ...
     int *nomemp;
{
  ...
  case P_IBTREE:
    ...
    ipage->inp[indx] = HOFFSET (ipage) -= BINTERNAL_SIZE (child_bi->len);
  ...
  case P_LDUP:
    ...
    {
    case B_KEYDATA:
      if (P_FREESPACE (ipage) < BINTERNAL_PSIZE (child_bk->len))
      {
        *nomemp = 1;
        return (0);
      }
      ipage->inp[indx] = HOFFSET (ipage) -= BINTERNAL_SIZE (child_bk->len);
      p = P_ENTRY (ipage, indx);
      ...
      break;
    case B_OVERFLOW:
      if (P_FREESPACE (ipage) < BINTERNAL_PSIZE (BOVERFLOW_SIZE))
      {
        *nomemp = 1;
        return (0);
      }
