...
#ifndef lint
static const char revid[] = "$Id: hash_upgrade.c,v 1.1.2.2 2000/09/14 03:13:21 ghutchis Exp $";
#endif /* not lint */
...
/*
 * CDB___ham_30_hashmeta --
 *      Upgrade the database from version 4/5 to version 6.
 *
 * PUBLIC: int CDB___ham_30_hashmeta __P((DB *, char *, u_int8_t *));
 */
int
CDB___ham_30_hashmeta(dbp, real_name, obuf)
	DB *dbp;
	char *real_name;
	u_int8_t *obuf;
{
	...
	if ((ret = CDB___os_fileid(dbenv, real_name, 1, newmeta.dbmeta.uid)) != 0)
		return (ret);
...
/*
 * CDB___ham_31_hashmeta --
 *      Upgrade the database from version 6 to version 7.
 *
 * PUBLIC: int CDB___ham_31_hashmeta
 * PUBLIC:      __P((DB *, char *, u_int32_t, DB_FH *, PAGE *, int *));
 */
int
CDB___ham_31_hashmeta(dbp, real_name, flags, fhp, h, dirtyp)
	DB *dbp;
	char *real_name;
	u_int32_t flags;
	DB_FH *fhp;
 ...
 *
 * PUBLIC: int CDB___ham_31_hash
 * PUBLIC:      __P((DB *, char *, u_int32_t, DB_FH *, PAGE *, int *));
 */
int
CDB___ham_31_hash(dbp, real_name, flags, fhp, h, dirtyp)
	DB *dbp;
	char *real_name;
	u_int32_t flags;
	DB_FH *fhp;
	...
	int *dirtyp;
{
	...
	for (indx = 0; indx < NUM_ENT(h); indx += 2) {
		...
		if (HPAGE_PTYPE(hk) == H_OFFDUP) {
			...
			tpgno = pgno;
			if ((ret = CDB___db_31_offdup(dbp, real_name, fhp,
