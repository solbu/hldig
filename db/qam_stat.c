...
#ifndef lint
static const char revid[] = "$Id: qam_stat.c,v 1.1.2.2 2000/09/14 03:13:23 ghutchis Exp $";
#endif /* not lint */
	...
	u_int32_t flags;
{
	...
	if (flags == DB_CACHED_COUNTS) {
		if ((ret = CDB___db_lget(dbc,
			...
			goto err;
		if ((ret =
		    CDB_memp_fget(dbp->mpf, &t->q_meta, 0, (PAGE **)&meta)) != 0)
			goto err;
	...
	if ((ret = CDB_memp_fput(dbp->mpf, meta, !F_ISSET(dbp, DB_AM_RDONLY))) != 0)
		goto err;
	(void)__LPUT(dbc, lock);
