...
int
CDB___ham_insdel_log (dbenv, txnid, ret_lsnp, flags,
                      opcode, fileid, pgno, ndx, pagelsn, key, data)
     DB_ENV *dbenv;
     DB_TXN *txnid;
     DB_LSN *ret_lsnp;
     uint32_t flags;
...
int
CDB___ham_newpage_log (dbenv, txnid, ret_lsnp, flags,
     ...
     DB_LSN *nextlsn;
{
  ...
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    ...
    + sizeof (*prevlsn)
    + sizeof (new_pgno)
    + sizeof (*pagelsn) + sizeof (next_pgno) + sizeof (*nextlsn);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
...
int
CDB___ham_splitmeta_log (dbenv, txnid, ret_lsnp, flags,
     ...
     DB_LSN *metalsn;
{
  ...
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    + sizeof (fileid)
    + sizeof (bucket)
    + sizeof (ovflpoint) + sizeof (spares) + sizeof (*metalsn);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
...
int
CDB___ham_splitdata_log (dbenv, txnid, ret_lsnp, flags,
...
int
CDB___ham_replace_log (dbenv, txnid, ret_lsnp, flags,
...
int
CDB___ham_newpgno_log (dbenv, txnid, ret_lsnp, flags,
     ...
     DB_LSN *metalsn;
{
  ...
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    ...
    + sizeof (old_type)
    + sizeof (old_pgno)
    + sizeof (new_type) + sizeof (*pagelsn) + sizeof (*metalsn);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
...
int
CDB___ham_ovfl_log (dbenv, txnid, ret_lsnp, flags,
     ...
     DB_LSN *metalsn;
{
  ...
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    ...
    + sizeof (start_pgno)
    + sizeof (npages)
    + sizeof (free_pgno) + sizeof (ovflpoint) + sizeof (*metalsn);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
...
int
CDB___ham_copypage_log (dbenv, txnid, ret_lsnp, flags,
...
int
CDB___ham_metagroup_log (dbenv, txnid, ret_lsnp, flags,
     ...
     DB_LSN *pagelsn;
{
  ...
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    + sizeof (fileid)
    + sizeof (bucket) + sizeof (pgno) + sizeof (*metalsn) + sizeof (*pagelsn);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
...
int
CDB___ham_groupalloc_log (dbenv, txnid, ret_lsnp, flags,
     ...
     uint32_t num;
{
  ...
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    ...
    + sizeof (pgno)
    + sizeof (*metalsn)
    + sizeof (*mmetalsn) + sizeof (start_pgno) + sizeof (num);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
     ...
     DB_ENV *dbenv;
{
  ...
  if ((ret = CDB___db_add_recovery (dbenv,
    ...
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___ham_newpage_print,
                                    DB_ham_newpage)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___ham_splitmeta_print,
                                    DB_ham_splitmeta)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___ham_splitdata_print,
                                    DB_ham_splitdata)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___ham_replace_print,
                                    DB_ham_replace)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___ham_newpgno_print,
                                    DB_ham_newpgno)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
    ...
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___ham_copypage_print,
                                    DB_ham_copypage)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___ham_metagroup_print,
                                    DB_ham_metagroup)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___ham_groupalloc_print,
                                    DB_ham_groupalloc)) != 0)
    return (ret);
  return (0);
}
