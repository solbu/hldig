     ...
     u_int32_t flags;
{
  ...
  else if ((ret = CDB___txn_ckp_read (data.data, &ckp_args)) != 0)
  {
    ...
    goto out;
           ...
           (ret =
            CDB_log_get (dbenv, &ckp_args->last_ckp, &data, DB_SET)) != 0)
    goto first;
  else
  ...
  if (dbenv->db_feedback != NULL)
  {
    ...
    if (last_lsn.file == ckp_lsn.file)
      nfiles = (float) (last_lsn.offset - ckp_lsn.offset) / dbenv->lg_max;
    else
  ...
  for (;;)
  {
    if (dbenv->tx_recover != NULL)
      ret = dbenv->tx_recover (dbenv, &data, &lsn, TXN_OPENFILES, txninfo);
    else
      ret = CDB___db_dispatch (dbenv, &data, &lsn, TXN_OPENFILES, txninfo);
  ...
  else if ((ret = CDB___log_findckp (dbenv, &first_lsn)) == DB_NOTFOUND)
  {
    /*
  ...
  {
    if (dbenv->tx_recover != NULL)
      ret = dbenv->tx_recover (dbenv, &data, &lsn, TXN_FORWARD_ROLL, txninfo);
    else
      ret = CDB___db_dispatch (dbenv, &data, &lsn, TXN_FORWARD_ROLL, txninfo);
    if (ret != 0)
    {
      if (ret != DB_TXN_CKP)
        goto msgerr;
