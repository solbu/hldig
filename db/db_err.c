...
#ifndef lint
static const char revid[] = "$Id: db_err.c,v 1.1.2.3 2000/09/17 01:35:04 ghutchis Exp $";
#endif /* not lint */
...
/*
	...
	have a finer granularity so that the only calls to CDB___db_err we
	...
	va_start(ap);
#endif
	CDB___db_real_log(dbenv, NULL, "db_err", 0, fmt, ap);
 ...
 *
 * PUBLIC: #ifdef __STDC__
 * PUBLIC: void CDB___db_logmsg __P((const DB_ENV *,
 * PUBLIC:     DB_TXN *, const char *, u_int32_t, const char *, ...));
 * PUBLIC: #else
 * PUBLIC: void CDB___db_logmsg();
 * PUBLIC: #endif
 */
void
#ifdef __STDC__
 ...
 *
 * PUBLIC: #ifdef __STDC__
 * PUBLIC: void CDB___db_real_log __P((const DB_ENV *,
 * PUBLIC:     DB_TXN *, const char *, u_int32_t, const char *, va_list ap));
 * PUBLIC: #else
 * PUBLIC: void CDB___db_real_log();
 * PUBLIC: #endif
 */
void
#ifdef __STDC__
CDB___db_real_log(const DB_ENV *dbenv, DB_TXN *txnid,
...
/*
 * CDB___db_unknown_flag -- report internal error
 *
 * PUBLIC: int CDB___db_unknown_flag __P((DB_ENV *, char *, u_int32_t));
 */
int
...
/*
 * CDB___db_unknown_type -- report internal error
 *
 * PUBLIC: int CDB___db_unknown_type __P((DB_ENV *, char *, u_int32_t));
 */
int
CDB___db_unknown_type(dbenv, routine, type)
	DB_ENV *dbenv;
	char *routine;
	u_int32_t type;
{
	CDB___db_err(dbenv, "%s: Unknown db type: 0x%x", routine, type);
	DB_ASSERT(0);
	return (EINVAL);
}

/*
 * CDB___db_child_active_err -- cannot do an update with active child
 *
 * PUBLIC: int CDB___db_child_active_err __P((DB_ENV *));
 */
int
CDB___db_child_active_err(dbenv)
	DB_ENV *dbenv;
{
	CDB___db_err(dbenv, "Child transaction is active");
	return (EPERM);
}
