...
#ifndef lint
static const char revid[] = "$Id: os_fsync.c,v 1.1.2.2 2000/09/14 03:13:22 ghutchis Exp $";
#endif /* not lint */
 ...
 *
 * PUBLIC: int CDB___os_fsync __P((DB_ENV *, DB_FH *));
 */
int
CDB___os_fsync(dbenv, fhp)
	DB_ENV *dbenv;
	DB_FH *fhp;
{
	int ret;
	...
	if (ret != 0) {
		...
		CDB___db_err(dbenv, "fsync %s", strerror(ret));
	}
	return (ret);
}
