...
#ifndef lint
static const char revid[] = "$Id: hash_stat.c,v 1.1.2.2 2000/09/14 03:13:21 ghutchis Exp $";
#endif /* not lint */
	...
	void *cookie;
{
	...
	for (bucket = 0; bucket <= hcp->hdr->max_bucket; bucket++) {
		...
		for (ret = CDB___ham_get_cpage(dbc, mode); ret == 0;
		    ret = CDB___ham_next_cpage(dbc, pgno, 0)) {
			...
			/* Call the callback on CDB_main pages. */
			if ((ret = callback(dbp,
		...
		if (hcp->page != NULL) {
			if ((ret = CDB___ham_put_page(dbc->dbp, hcp->page, 0)) != 0)
				return (ret);
			hcp->page = NULL;
		}
	...
	int *putp;
{
	...
	*putp = 0;
	...
	case P_LDUP:
		...
		bstat.bt_ndata = 0;
		CDB___bam_stat_callback(dbp, pagep, &bstat, putp);
		sp->hash_dup++;
	...
	default:
		return (CDB___db_unknown_type(dbp->dbenv,
		     "__ham_stat_callback", TYPE(pagep)));
	}
