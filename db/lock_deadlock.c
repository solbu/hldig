     ...
     int *abortp;
{
  ...
  /* Validate arguments. */
  if ((ret =
       CDB___db_fchk (dbenv, "CDB_lock_detect", flags,
                      DB_LOCK_CONFLICT)) != 0)
    return (ret);
  ...
  for (; *deadp != NULL; deadp++)
  {
    ...
    case DB_LOCK_OLDEST:
      ...
      for (i = killid + 1; i < nlockers; i++)
        if (ISSET_MAP (*deadp, i) && idmap[i].id < idmap[killid].id)
          killid = i;
      break;
    ...
    if ((ret = CDB___dd_abort (dbenv, &idmap[killid])) != 0)
    {
      ...
      CDB___db_err (dbenv, "Aborting locker %lx", (u_long) idmap[killid].id);
  }
         ...
         op != NULL; op = SH_TAILQ_NEXT (op, links, __db_lockobj))
    {
           ...
           lp != NULL; lp = SH_TAILQ_NEXT (lp, links, __db_lock))
      {
        ...
        if (lockerp->dd_id == DD_INVALID_ID)
          dd = ((DB_LOCKER *)
                R_ADDR (&lt->reginfo, lockerp->master_locker))->dd_id;
        else
          dd = lockerp->dd_id;
        id_array[dd].valid = 1;
  ...
  for (id = 0; id < count; id++)
  {
    ...
    if (child != NULL)
    {
      do
      {
        ...
        if (lp != NULL && lp->status == DB_LSTAT_WAITING)
        {
     ...
     locker_info *info;
{
  ...
  if ((ret = CDB___lock_getlocker (lt,
                                   info->last_locker_id, ndx, 0,
                                   &lockerp)) != 0 || lockerp == NULL)
  {
    if (ret == 0)
      ret = EINVAL;
    goto out;
...
#define  MSGBUF_LEN ((nlockers + 1) * 10 + 64)
  ...
  for (mymap = bitmap, i = 0; i < nlockers; i++, mymap += nentries)
  {
    ...
    for (j = 0; j < nlockers; j++)
      if (ISSET_MAP (mymap, j))
        sprintf (msgbuf, "%s %lx", msgbuf, (u_long) idmap[j].id);
    (void) sprintf (msgbuf, "%s %lu", msgbuf, (u_long) idmap[i].last_lock);
