...
#ifndef lint
static const char revid[] = "$Id: mp_stat.c,v 1.1.2.2 2000/09/14 03:13:22 ghutchis Exp $";
#endif /* not lint */
	...
	void *(*db_malloc) __P((size_t));
{
	...
	if (gspp != NULL) {
		*gspp = NULL;
		...
		if ((ret = CDB___os_calloc(dbenv, 1, sizeof(**gspp), gspp)) != 0)
			return (ret);
		sp = *gspp;
	...
	if (fspp != NULL) {
		*fspp = NULL;
		...
		if ((ret = CDB___os_malloc(dbenv, len, db_malloc, fspp)) != 0)
			return (ret);
		...
		for (tfsp = *fspp,
			...
			if ((ret =
			    CDB___os_malloc(dbenv, len, db_malloc, tfsp)) != 0)
				return (ret);
			**tfsp = mfp->stat;
			(*tfsp)->file_name = (char *)
	...
	u_int32_t flags;
{
	...
	if (LF_ISSET(MPOOL_DUMP_MEM))
		CDB___db_shalloc_dump(reginfo->addr, fp);
}
