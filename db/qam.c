     ...
     DBT *data;
{
  ...
  if (F_ISSET (data, DB_DBT_PARTIAL))
  {
    ...
    if (DB_LOGGING (dbc) || !F_ISSET (qp, QAM_VALID))
    {
      ...
      if ((ret = CDB___os_malloc (t->re_len, NULL, &datap->data)) != 0)
        return (ret);
      alloced = 1;
      datap->size = t->re_len;
...
no_partial:
  if (DB_LOGGING (dbc))
  {
    ...
    if ((ret = CDB___qam_add_log (dbp->dbenv, dbc->txn, &LSN (pagep),
                                  0, dbp->log_fileid, &LSN (pagep),
                                  pagep->pgno, indx, recno, datap, qp->flags,
     ...
     uint32_t flags;
{
  ...
  if ((ret = CDB___db_cputchk (dbc->dbp, key, data, flags,
                               ...
                               ((QUEUE_CURSOR *) dbc->internal)->recno !=
                               RECNO_OOB)) != 0)
    return (ret);
     ...
     uint32_t flags;
{
  ...
  if ((ret = CDB___db_lget (dbc,
                            0, cp->recno, DB_LOCK_WRITE, DB_LOCK_RECORD,
                            &lock)) != 0)
    return (ret);

  if ((ret = CDB___qam_position (dbc,
                                 &cp->recno, DB_LOCK_WRITE, cp->start,
                                 &exact)) != 0)
  {
  ...
  if ((t_ret = __LPUT (dbc, cp->lock)) != 0 && ret == 0)
    ret = t_ret;
  if ((t_ret = CDB_memp_fput (dbp->mpf, cp->page, DB_MPOOL_DIRTY))
      && ret == 0)
    ret = t_ret;
  cp->lock = lock;
  ...
  if (opcode != 0 && DB_LOGGING (dbc))
  {
    ret = CDB___qam_mvptr_log (dbp->dbenv, dbc->txn, &meta->dbmeta.lsn,
                               0, opcode, dbp->log_fileid, meta->first_recno,
     ...
     uint32_t flags;
{
  ...
  if ((ret = CDB___db_putchk (dbp,
                              key, data, flags, F_ISSET (dbp, DB_AM_RDONLY),
                              0)) != 0)
    goto done;
  ...
  if ((ret = CDB___db_lget (dbc,
                            1, recno, DB_LOCK_WRITE, DB_LOCK_RECORD,
                            &lock)) != 0)
    goto done;
  ...
  if ((ret = CDB___qam_pitem (dbc, page,
                              QAM_RECNO_INDEX (dbp, pg, start, recno), recno,
                              data)) != 0)
  {
     ...
     DBC *dbc;
{
  ...
  if (DB_LOGGING (dbc))
  {
    ...
    if ((ret = CDB___qam_del_log (dbp->dbenv, dbc->txn,
                                  &LSN (pagep), 0, dbp->log_fileid,
                                  &LSN (pagep), pagep->pgno, cp->indx,
                                  cp->recno)) != 0)
      goto err1;
  }
...
err1:
  if ((t_ret =
       CDB_memp_fput (dbp->mpf, cp->page,
                      ret == 0 ? DB_MPOOL_DIRTY : 0)) != 0)
     ...
     uint32_t flags;
{
  ...
  DEBUG_LREAD (dbc_orig, dbc_orig->txn, "qam_c_get",
               flags == DB_SET
               || flags == DB_SET_RANGE ? key : NULL, NULL, flags);
...
retry:                         /* Update the record number. */
    ...
    if (with_delete && first < meta->cur_recno)
    {
      if (DB_LOGGING (dbc))
        CDB___qam_incfirst_log (dbp->dbenv, dbc->txn,
                                &LSN (meta), 0, dbp->log_fileid, first);
      meta->first_recno++;
      meta_dirty = 1;
    }
  ...
  if ((ret = CDB___db_lget (dbc, 0, cp->recno, lock_mode,
                            with_delete ? DB_LOCK_NOWAIT | DB_LOCK_RECORD :
                            DB_LOCK_RECORD, &lock)) == DB_LOCK_NOTGRANTED)
  {
    /*
  ...
  if ((!with_delete && is_first) || flags == DB_LAST)
  {
    if ((ret = CDB___db_lget (dbc, 0, metapno, lock_mode, 0, &metalock)) != 0)
      goto err;
    if (cp->recno != (is_first ? meta->first_recno : meta->cur_recno))
    {
      __LPUT (dbc, lock);
      if (is_first)
        flags = DB_FIRST;
  ...
  if (with_delete)
  {
    ...
    if (cp->recno != first)
    {
      ...
      else if (meta->first_recno > skipped)
      {
        ...
        do
        {
          ...
          if ((t_ret =
               CDB___qam_position (dbc, &skipped,
                                   DB_LOCK_READ, start, &exact)) != 0)
          {
            (void) __LPUT (dbc, lock);
            goto err1;
          }
          if ((t_ret = CDB_memp_fput (dbp->mpf, cp->page, put_mode)) != 0)
            goto err1;
        ...
        t_ret = 0;
        if ((t_ret = CDB___db_lget (dbc, 0, metapno,
...
err:
  if (meta)
  {
    ...
    /* release the meta page */
    t_ret = CDB_memp_fput (dbp->mpf, meta, meta_dirty ? DB_MPOOL_DIRTY : 0);
     ...
     DBC *orig_dbc, *new_dbc;
{
  ...
  return (CDB___db_lget (new_dbc,
                         0, new->recno, new->lock_mode, DB_LOCK_RECORD,
                         &new->lock));
}
