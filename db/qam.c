	...
	db_recno_t *rep, *start;
{
	...
	*start = meta->start;
	...
	ret = CDB_memp_fput(dbp->mpf, meta, 0);
	...
	u_int32_t flags;
{
	...
	if ((t_ret
	    = CDB_memp_fput(dbp->mpf, page, DB_MPOOL_DIRTY)) != 0 && ret == 0)
		ret = t_ret;


	/* Return the record number to the user. */
	 ret = CDB___db_retcopy(dbp, key,
...
retry:	/* Update the record number. */
	...
	case DB_SET_RANGE:
		...
		break;
	default:
		ret = CDB___db_unknown_flag(dbp->dbenv, "__qam_c_get", flags);
		goto err;
	}
	...
	if (flags == DB_GET_BOTH) {
		...
		if ((ret = CDB___bam_defcmp(data, &tmp)) != 0) {
			ret = DB_NOTFOUND;
			goto err1;
		}
	}
	...
	DBC *dbc;
{
	...
	if (cp == NULL) {
		if ((ret =
		    CDB___os_calloc(dbp->dbenv, 1, sizeof(QUEUE_CURSOR), &cp)) != 0)
			return (ret);
