     ...
     uint32_t flags;
{
  ...
  case P_QAMDATA:              /* Should be meta->start. */
         ...
         recno++, i++, qp = QAM_GET_RECORD (dbp, h, i))
    {
      ...
      fprintf (fp, "%s", F_ISSET (qp, QAM_VALID) ? "\t" : "       D");
      fprintf (fp, "[%03lu] %4lu ", (u_long) recno, (u_long) qp - (u_long) h);
  ...
  for (i = 0; i < NUM_ENT (h); i++)
  {
    ...
    case P_LBTREE:
      sp = P_ENTRY (h, i);
      deleted = i % 2 == 0 && B_DISSET (GET_BKEYDATA (h, i + O_INDX)->type);
      break;
    case P_LRECNO:
    case P_DUPLICATE:
      sp = P_ENTRY (h, i);
      ...
      break;
    default:
      fprintf (fp, "ILLEGAL PAGE ITEM: %lu\n", (u_long) TYPE (h));
      ret = EINVAL;
      continue;
    }
     ...
     int die;
{
  ...
  for (i = 0; i < NUM_ENT (h); i++)
  {
    ...
    case P_HASH:
      type = HPAGE_TYPE (h, i);
      if (type != H_OFFDUP &&
          type != H_DUPLICATE && type != H_KEYDATA && type != H_OFFPAGE)
      {
    ...
    case P_IBTREE:
      ...
      if (B_TYPE (bi->type) != B_KEYDATA &&
          B_TYPE (bi->type) != B_DUPLICATE && B_TYPE (bi->type) != B_OVERFLOW)
      {
        fprintf (fp, "ILLEGAL BINTERNAL TYPE: %lu\n",
                 (u_long) B_TYPE (bi->type));
        goto bad;
    ...
    case P_DUPLICATE:
      ...
      if (B_TYPE (bk->type) != B_KEYDATA &&
          B_TYPE (bk->type) != B_DUPLICATE && B_TYPE (bk->type) != B_OVERFLOW)
      {
        fprintf (fp,
                 "ILLEGAL DUPLICATE/LBTREE/LRECNO TYPE: %lu\n",
                 (u_long) B_TYPE (bk->type));
