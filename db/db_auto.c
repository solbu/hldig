...
int
CDB___db_addrem_log (dbenv, txnid, ret_lsnp, flags,
                     opcode, fileid, pgno, indx, nbytes, hdr, dbt, pagelsn)
     DB_ENV *dbenv;
     DB_TXN *txnid;
     DB_LSN *ret_lsnp;
     u_int32_t flags;
...
int
CDB___db_split_log (dbenv, txnid, ret_lsnp, flags,
...
int
CDB___db_big_log (dbenv, txnid, ret_lsnp, flags,
...
int
CDB___db_ovref_log (dbenv, txnid, ret_lsnp, flags, fileid, pgno, adjust, lsn)
     DB_ENV *dbenv;
     DB_TXN *txnid;
     DB_LSN *ret_lsnp;
     u_int32_t flags;
     ...
     DB_LSN *lsn;
{
  ...
  if (txnid == NULL)
  {
    ...
    lsnp = &txnid->last_lsn;
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    + sizeof (fileid) + sizeof (pgno) + sizeof (adjust) + sizeof (*lsn);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
...
int
CDB___db_relink_log (dbenv, txnid, ret_lsnp, flags,
     ...
     DB_LSN *lsn_next;
{
  ...
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    ...
    + sizeof (pgno)
    + sizeof (*lsn)
    + sizeof (prev) + sizeof (*lsn_prev) + sizeof (next) + sizeof (*lsn_next);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
...
int
CDB___db_addpage_log (dbenv, txnid, ret_lsnp, flags,
     ...
     DB_LSN *nextlsn;
{
  ...
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    + sizeof (fileid)
    + sizeof (pgno) + sizeof (*lsn) + sizeof (nextpgno) + sizeof (*nextlsn);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
...
int
CDB___db_debug_log (dbenv, txnid, ret_lsnp, flags,
...
int
CDB___db_noop_log (dbenv, txnid, ret_lsnp, flags, fileid, pgno, prevlsn)
     DB_ENV *dbenv;
     DB_TXN *txnid;
     DB_LSN *ret_lsnp;
     u_int32_t flags;
     ...
     DB_LSN *prevlsn;
{
  ...
  if (txnid == NULL)
  {
    ...
    lsnp = &txnid->last_lsn;
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    + sizeof (fileid) + sizeof (pgno) + sizeof (*prevlsn);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
     ...
     DB_ENV *dbenv;
{
  ...
  if ((ret = CDB___db_add_recovery (dbenv,
    ...
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___db_relink_print,
                                    DB_db_relink)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___db_addpage_print,
                                    DB_db_addpage)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
     ...
     DB_ENV *dbenv;
{
  ...
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___db_addrem_recover,
                                    DB_db_addrem)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___db_split_recover,
                                    DB_db_split)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
