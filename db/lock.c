...
#ifndef lint
static const char revid[] = "$Id: lock.c,v 1.1.2.2 2000/09/14 03:13:21 ghutchis Exp $";
#endif /* not lint */
	...
	DB_LOCKREQ *list, **elistp;
{
	...
	for (i = 0, ret = 0; i < nlist && ret == 0; i++)
		...
		case DB_LOCK_INHERIT:
			 ...
			 */
			ndx = CDB___lock_locker_hash(locker) % region->table_size;
			if ((ret = CDB___lock_getlocker(lt,
				...
				if (ret == 0 && sh_locker != NULL)
					ret = EACCES;
				CDB___db_err(dbenv, __db_locker_invalid);
				break;
			}

			/* Make sure we are a child transaction. */
			if (sh_locker->parent_locker == INVALID_ROFF) {
				CDB___db_err(dbenv, "Not a child transaction");
				ret = EINVAL;
				break;
			}
			sh_parent = (DB_LOCKER *)
	...
	DB_LOCK *lock;
{
	...
	if (newl == NULL) {
		CDB___db_err(dbenv, __db_lock_err, "locks");
		return (ENOMEM);
	}
	...
	locker_ndx = CDB___lock_locker_hash(locker) % region->table_size;
	if ((ret =
	...
	if (lp == NULL)
		...
		if (SH_LIST_FIRST(&sh_locker->heldby, __db_lock) == NULL
			...
			CDB___lock_freelocker( lt, region, sh_locker, locker_ndx);
		region->nnowaits++;
		goto err;
	}
 ...
 * __lock_put_nolock, __lock_put_internal and CDB___lock_downgrade.
 */
int
	...
	int *runp;
{
	...
	if (lock->gen != lockp->gen) {
		CDB___db_err(dbenv, __db_lock_invalid, "CDB_lock_put");
		return(EACCES);
	}
	...
	u_int32_t flags;
{
	...
	lockp = (struct __db_lock *)R_ADDR(&lt->reginfo, lock->off);
	if (lock->gen != lockp->gen) {
		CDB___db_err(dbenv, __db_lock_invalid, "lock_downgrade");
		ret = EACCES;
		goto out;
	}
	...
	int *freed;
{
	...
	indx = CDB___lock_locker_hash(locker) % region->table_size;
	...
	if ((ret = CDB___lock_getlocker(lt,
		...
		if (ret == 0)
			ret = EACCES;
		CDB___db_err(lt->dbenv, __db_locker_invalid);
		goto freelock;
	}
	...
	u_int32_t pid, id;
{
	...
	ndx = CDB___lock_locker_hash(pid) % region->table_size;
	 ...
	 */
	ndx = CDB___lock_locker_hash(id) % region->table_size;
	if ((ret = CDB___lock_getlocker(dbenv->lk_handle,
	    id, ndx, 1, &lockerp)) != 0)
		goto err;
	...
	u_int32_t locker;
{
	...
	if (SH_LIST_FIRST(&sh_locker->heldby, __db_lock) != NULL) {
		ret = EINVAL;
		CDB___db_err(dbenv, "Freeing locker with locks");
		goto freelock;
	}
	...
	DB_LOCKER **retp;
{
	...
	if (sh_locker == NULL && create) {
		...
		if ((sh_locker = SH_TAILQ_FIRST(
			...
			CDB___db_err(lt->dbenv, __db_lock_err, "locker entries");
			return (ENOMEM);
		}
		SH_TAILQ_REMOVE(
		    &region->free_lockers, sh_locker, links, __db_locker);
	...
	DB_LOCKOBJ **retp;
{
	...
	if (sh_obj == NULL && create) {
		...
		if ((sh_obj =
			...
			CDB___db_err(lt->dbenv, __db_lock_err, "object entries");
			ret = ENOMEM;
			goto err;
		}
		...
		if (obj->size <= sizeof(sh_obj->objdata))
			p = sh_obj->objdata;
		else if ((ret = CDB___db_shalloc(
		    lt->reginfo.addr, obj->size, 0, &p)) != 0) {
			CDB___db_err(dbenv, "No space for lock object storage");
			goto err;
		}
	...
	for (lp_w = SH_TAILQ_FIRST(&obj->waiters, __db_lock),
		...
		for (lp_h = SH_TAILQ_FIRST(&obj->holders, __db_lock);
			...
			if (lp_h->holder != lp_w->holder &&
				...
				locker_ndx = CDB___lock_locker_hash(lp_w->holder)
				    % region->table_size;
				if ((CDB___lock_getlocker(lt, lp_w->holder,
