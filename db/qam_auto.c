...
int
CDB___qam_inc_log (dbenv, txnid, ret_lsnp, flags, fileid, lsn)
     DB_ENV *dbenv;
     DB_TXN *txnid;
     DB_LSN *ret_lsnp;
     u_int32_t flags;
...
int
CDB___qam_incfirst_log (dbenv, txnid, ret_lsnp, flags, fileid, recno)
     DB_ENV *dbenv;
     DB_TXN *txnid;
     DB_LSN *ret_lsnp;
     u_int32_t flags;
...
int
CDB___qam_mvptr_log (dbenv, txnid, ret_lsnp, flags,
     ...
     DB_LSN *metalsn;
{
  ...
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    ...
    + sizeof (old_first)
    + sizeof (new_first)
    + sizeof (old_cur) + sizeof (new_cur) + sizeof (*metalsn);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
...
int
CDB___qam_del_log (dbenv, txnid, ret_lsnp, flags,
     ...
     db_recno_t recno;
{
  ...
  logrec.size = sizeof (rectype) + sizeof (txn_num) + sizeof (DB_LSN)
    + sizeof (fileid)
    + sizeof (*lsn) + sizeof (pgno) + sizeof (indx) + sizeof (recno);
  if ((ret = CDB___os_malloc (logrec.size, NULL, &logrec.data)) != 0)
    return (ret);
...
int
CDB___qam_add_log (dbenv, txnid, ret_lsnp, flags,
                   fileid, lsn, pgno, indx, recno, data, vflag, olddata)
     DB_ENV *dbenv;
     DB_TXN *txnid;
     DB_LSN *ret_lsnp;
     u_int32_t flags;
     ...
     DB_ENV *dbenv;
{
  ...
  if ((ret = CDB___db_add_recovery (dbenv,
    ...
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___qam_incfirst_print,
                                    DB_qam_incfirst)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
                                    CDB___qam_mvptr_print,
                                    DB_qam_mvptr)) != 0)
    return (ret);
  if ((ret = CDB___db_add_recovery (dbenv,
