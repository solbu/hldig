...
#ifndef lint
static const char revid[] = "$Id: os_method.c,v 1.1.2.3 2000/09/17 01:35:07 ghutchis Exp $";
#endif /* not lint */
...
int
CDB_db_env_set_func_close(func_close)
...
int
CDB_db_env_set_func_dirfree(func_dirfree)
...
int
CDB_db_env_set_func_dirlist(func_dirlist)
...
int
CDB_db_env_set_func_exists(func_exists)
...
int
CDB_db_env_set_func_free(func_free)
...
int
CDB_db_env_set_func_fsync(func_fsync)
...
int
CDB_db_env_set_func_ioinfo(func_ioinfo)
	int (*func_ioinfo)
...
int
CDB_db_env_set_func_malloc(func_malloc)
...
int
CDB_db_env_set_func_map(func_map)
...
int
CDB_db_env_set_func_open(func_open)
...
int
CDB_db_env_set_func_read(func_read)
...
int
CDB_db_env_set_func_realloc(func_realloc)
...
int
CDB_db_env_set_func_rename(func_rename)
...
int
CDB_db_env_set_func_seek(func_seek)
...
int
CDB_db_env_set_func_sleep(func_sleep)
...
int
CDB_db_env_set_func_unlink(func_unlink)
...
int
CDB_db_env_set_func_unmap(func_unmap)
...
int
CDB_db_env_set_func_write(func_write)
...
int
CDB_db_env_set_func_yield(func_yield)
...
int
CDB_db_env_set_mutexlocks(onoff)
	int onoff;
{
...
int
CDB_db_env_set_pageyield(onoff)
	int onoff;
{
...
int
CDB_db_env_set_panicstate(onoff)
	int onoff;
{
...
int
CDB_db_env_set_tas_spins(tas_spins)
	u_int32_t tas_spins;
{
