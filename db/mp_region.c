...
#ifndef lint
static const char revid[] = "$Id: mp_region.c,v 1.1.2.4 2000/09/17 01:35:07 ghutchis Exp $";
#endif /* not lint */
	...
	DB_ENV *dbenv;
{
	...
	if ((ret = CDB___os_calloc(dbenv, 1, sizeof(*dbmp), &dbmp)) != 0)
		return (ret);
	...
	if ((ret = CDB___db_r_attach(dbenv, &reginfo, reg_size)) != 0)
		goto err;
	...
	int reginfo_off, htab_buckets;
{
	...
	if (reginfo_off == 0) {
		...
		mp->nreg = dbmp->nreg;
		if ((ret = CDB___db_shalloc(dbmp->reginfo[0].addr,
...
mem_err:CDB___db_err(dbenv, "Unable to allocate memory for mpool region");
	...
	DB_ENV *dbenv;
{
	...
	CDB___os_free(dbmp->reginfo, dbmp->nreg * sizeof(*dbmp->reginfo));
	CDB___os_free(dbmp, sizeof(*dbmp));
