     ...
     void *info;
{
  ...
  if (redo)
  {
    ...
    if (rootsplit)
    {
      ...
      p_update = CDB_log_compare (&LSN (pp), &LSN (argp->pg.data)) == 0;
    }
    else if (lp == NULL)
    {
    ...
    if (lp == NULL && (ret =
                       CDB_memp_fget (mpf, &argp->left, DB_MPOOL_CREATE,
                                      &lp)) != 0)
    {
      (void) CDB___db_pgerr (file_dbp, argp->left);
      lp = NULL;
    ...
    if (!rootsplit && !IS_ZERO_LSN (argp->nlsn))
    {
      ...
      if (CDB_log_compare (&LSN (np), &argp->nlsn) == 0)
      {
        ...
        np->lsn = *lsnp;
        if ((ret = CDB_memp_fput (mpf, np, DB_MPOOL_DIRTY)) != 0)
          goto out;
        np = NULL;
      }
    }
  ...
  lrundo:if ((rootsplit && lp != NULL) || rp != NULL)
    {
      if (rootsplit && lp != NULL && CDB_log_compare (lsnp, &LSN (lp)) == 0)
      {
        lp->lsn = argp->llsn;
        if ((ret = CDB_memp_fput (mpf, lp, DB_MPOOL_DIRTY)) != 0)
          goto out;
      ...
      if (rp != NULL && CDB_log_compare (lsnp, &LSN (rp)) == 0)
      {
        rp->lsn = argp->rlsn;
        if ((ret = CDB_memp_fput (mpf, rp, DB_MPOOL_DIRTY)) != 0)
          goto out;
     ...
     void *info;
{
  ...
  else if (cmp_n == 0 && !redo)
  {
    ...
    if ((ret = CDB___db_pitem (dbc, pagep, 0,
                               argp->rootent.size, &argp->rootent,
                               NULL)) != 0)
      goto out;
     ...
     void *info;
{
  ...
  if (cmp_p == 0 && redo)
  {
    ...
    if ((ret = CDB___bam_adjindx (dbc,
                                  pagep, argp->indx, argp->indx_copy,
                                  argp->is_insert)) != 0)
      goto err;
     ...
     void *info;
{
  ...
  {
    /* Need to redo update described. */
    if (file_dbp->type == DB_BTREE && F_ISSET (file_dbp, DB_BT_RECNUM))
    {
