     ...
     uint32_t flags;
{
  ...
  if ((ret =
       CDB___db_cursorchk (dbp, flags, F_ISSET (dbp, DB_AM_RDONLY))) != 0)
    return (ret);
  ...
  else
  {
    ...
    if (F_ISSET (dbp->dbenv, DB_ENV_CDB | DB_ENV_LOCKING))
    {
      ...
      if (!F_ISSET (dbp->dbenv, DB_ENV_THREAD) &&
        ...
        dbc->lid = adbc->lid;
      else if ((ret = CDB_lock_id (dbp->dbenv, &dbc->lid)) != 0)
        goto err;
     ...
     uint32_t flags;
{
  ...
  if (flags == DB_NOOVERWRITE)
  {
    ...
    if ((ret = dbc->c_get (dbc, key, &tdata, DB_SET |
                           (F_ISSET (dbp->dbenv, DB_ENV_LOCKING) ? DB_RMW :
                            0))) == 0)
      ret = DB_KEYEXIST;
     ...
     PAGE *page;
{
  ...
  if (pgno == PGNO_BASE_MD)
  {
    ...
    ret = CDB___crdel_metapage_log (dbp->dbenv,
                                    ...
                                    dbp->log_fileid, &name_dbt, pgno,
                                    &page_dbt);
  }
  else
    ret = CDB___crdel_metasub_log (dbp->dbenv, dbp->open_txn,
                                   &new_lsn, 0, dbp->log_fileid, pgno,
                                   &page_dbt, lsn);
