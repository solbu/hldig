     ...
     DB_ENV *dbenv;
{
  ...
  {
    CDB___os_free (dbenv->lk_conflicts, dbenv->lk_modes * dbenv->lk_modes);
     ...
     int lk_modes;
{
  ...
  if ((ret =
       CDB___os_malloc (lk_modes * lk_modes, NULL,
                        &dbenv->lk_conflicts)) != 0)
    return (ret);
     ...
     DB_ENV *dbenv;
{
  ...
  if (dbenv->lk_detect != DB_LOCK_NORUN)
  {
    ...
    {
      CDB___db_err (dbenv, "lock_open: incompatible deadlock detector mode");
      ret = EINVAL;
      goto err;
    }
     ...
     DB_LOCKTAB *lt;
{
  ...
  if ((ret = CDB___db_shalloc (lt->reginfo.addr,
                               sizeof (DB_LOCKREGION), 0,
                               &lt->reginfo.primary)) != 0)
    return (ret);
  ...
  if ((ret =
       CDB___db_shalloc (lt->reginfo.addr, lk_modes * lk_modes, 0,
                         &addr)) != 0)
    return (ret);
  ...
  if ((ret = __db_mutex_init (dbenv, (MUTEX *) addr,
                              R_OFFSET (&lt->reginfo,
                                        addr) + DB_FCNTL_OFF_LOCK, 0)) != 0)
    return (ret);

  /* Allocate room for the mutex tables and initialize them. */
  if ((ret = CDB___db_shalloc (lt->reginfo.addr,
                               nelements * sizeof (MUTEX), MUTEX_ALIGN,
                               &addr)) != 0)
    return (ret);
  ...
  {
    if ((ret = CDB___db_shalloc (lt->reginfo.addr,
                                 sizeof (struct __db_lock), MUTEX_ALIGN,
                                 &lp)) != 0)
      return (ret);
    lp->status = DB_LSTAT_FREE;
    if ((ret = __db_mutex_init (dbenv, &lp->mutex,
                                R_OFFSET (&lt->reginfo,
                                          &lp->mutex) + DB_FCNTL_OFF_LOCK,
                                MUTEX_SELF_BLOCK)) != 0)
      return (ret);
  ...
  for (i = 0; i < region->maxlocks; ++i)
  {
    if ((ret = CDB___db_shalloc (lt->reginfo.addr,
      ...
      return (ret);
    SH_TAILQ_INSERT_HEAD (&region->free_objs, op, links, __db_lockobj);
  }
  ...
  if (LF_ISSET (LOCK_DUMP_CONF))
  {
    ...
    for (i = 0; i < lrp->nmodes; i++)
    {
      for (j = 0; j < lrp->nmodes; j++)
        fprintf (fp, "%lu\t", (u_long) lt->conflicts[i * lrp->nmodes + j]);
      fprintf (fp, "\n");
    }
  }
