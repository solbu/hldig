...
#define  OKFLAGS_CDB              \
  ...
  if (LF_ISSET (DB_RECOVER | DB_RECOVER_FATAL))
  {
    if ((ret = CDB_db_env_create (&rm_dbenv, 0)) != 0)
      return (ret);
    if ((ret = dbenv->remove (rm_dbenv, db_home, db_config, DB_FORCE)) != 0)
      return (ret);
  }
...
#define  CONFIG_NAME  "/DB_CONFIG"
    if (strlen (dbenv->db_home) + strlen (CONFIG_NAME) + 1 > sizeof (buf))
    {
      ret = ENAMETOOLONG;
      return (ret);
    }
    ...
    if ((fp = fopen (buf, "r")) != NULL)
    {
      while (fgets (buf, sizeof (buf), fp) != NULL)
      {
        if ((lp = strchr (buf, '\n')) == NULL)
        {
          CDB___db_err (dbenv, "%s: line too long", CONFIG_NAME);
          (void) fclose (fp);
          ret = EINVAL;
          return (ret);
        }
        *lp = '\0';
        if (buf[0] == '\0' || buf[0] == '#' || isspace ((int) buf[0]))
          continue;
  ...
  if (dbenv->db_tmp_dir == NULL
      && (ret = CDB___os_tmpdir (dbenv, flags)) != 0)
    return (ret);
     ...
     DB_ENV *dbenv;
{
  ...
  if (dbenv->db_data_dir != NULL)
  {
    for (p = dbenv->db_data_dir; *p != NULL; ++p)
      CDB___os_freestr (*p);
    CDB___os_free (dbenv->db_data_dir, dbenv->data_cnt * sizeof (char **));
  ...
  if (dbenv->dtab != NULL)
  {
    CDB___os_free (dbenv->dtab, dbenv->dtab_size * sizeof (dbenv->dtab[0]));
    dbenv->dtab = NULL;
    dbenv->dtab_size = 0;
  }
...
retry:switch (appname)
  ...
  case DB_APP_DATA:
    if (dir != NULL)
    {
      CDB___db_err (dbenv, "DB_APP_DATA: illegal directory specification");
      return (EINVAL);
    }
  ...
  tmp:if (dbenv == NULL || !F_ISSET (dbenv, DB_ENV_OPEN_CALLED))
    ...
    (a == NULL ? 0 : strlen (a) + 1) +
    (b == NULL ? 0 : strlen (b) + 1) +
    (c == NULL ? 0 : strlen (c) + 1) + (file == NULL ? 0 : strlen (file) + 1);
  ...
  *trv = PATH_SEPARATOR[0];
  ...
  for (;;)
  {
    if ((ret = CDB___os_open (path,
                              tmp_oflags | DB_OSO_CREATE | DB_OSO_EXCL, mode,
                              fhp)) == 0)
      return (0);
    ...
    if (ret != EEXIST)
    {
      CDB___db_err (dbenv, "tmp_open: %s: %s", path, CDB_db_strerror (ret));
      return (ret);
    }
