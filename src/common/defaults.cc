...
" }, \
{ "debug_override", "-1",  \
	"number", "all", "", "4.0", "Debugging output", "debug_override: 4", " \
    This setting will have the final say in what the debug level actually is. \
    One caveat: the override must be set to zero or greater to have any \
    effect. This is primarily useful in dealing with htdig through an API \
    (which might have the debug level hardcoded), so the debugging level \
    can be changed from the config file without recompiling any code. \
" }, \
