...
#else
#error this will not work.. need COMPILE_DL_HTDIG
#endif
//static int le_htdig;
 ...
 *
 *  TODO recieve configFile, debug, debugFile, logFile as parameters.
 * 
 */
PHP_FUNCTION(htsearch_open)
{
    ...
    if (zend_hash_find(config_array_ht, "debugFile", sizeof("debugFile"), 
                        (void **) &pvalue) == SUCCESS)
    {
        convert_to_string_ex(pvalue);
        sprintf(htsearch_params.debugFile, Z_STRVAL_PP(pvalue));
    }
    else
    {
        php_error(E_WARNING, "'debugFile' key not in array parameter");
        return;
    }
    ...
    int val = htsearch_open(&htsearch_params);
