...
//
int
DocumentDB::Open (const String & filename, const String & indexfilename,
                  const String & headname)
{
  ...
  if (dbf->OpenReadWrite (filename, 0666) == OK)
  {
    ...
    String key ((char *) &specialRecordNumber, sizeof specialRecordNumber);
...
//
int
DocumentDB::Read (const String & filename, const String & indexfilename,
                  const String & headfilename)
{
  // If the database is already open, we'll close it
  // We might be opening this object with a new filename, so we'll be safe
  Close ();
...
//
int
DocumentDB::Add (DocumentRef & doc)
{
  int docID = doc.DocID ();
...
//
int
DocumentDB::ReadExcerpt (DocumentRef & ref)
{
  String data;
  int docID = ref.DocID ();
  String key ((char *) &docID, sizeof docID);
...
//
DocumentRef *
DocumentDB::operator [] (int docID)
{
  String
    data;
  String
  key ((char *) &docID, sizeof docID);
...
//
DocumentRef *
DocumentDB::operator [] (const String & u)
{
  String
    data;
  String
...
//
int
DocumentDB::DumpDB (const String & filename, int verbose)
{
  DocumentRef *ref;
...
//
int
DocumentDB::LoadDB (const String & filename, int verbose)
{
  FILE *input;
  String docKey (sizeof (int));
  DocumentRef ref;
