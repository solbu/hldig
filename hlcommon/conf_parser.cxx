...
#line 69 "conf_parser.yxx"
    {
      // name: value
      ((HtConfiguration *) YYPARSE_PARAM)->AddParsed (yyvsp[0].ConfLine->name,
                                                      yyvsp[0].ConfLine->
                                                      value);
#ifdef DEBUG
      if (sn_debug >= 2)
      {
        cout << "Added to conf: " << yyvsp[0].ConfLine->
          name << ":" << yyvsp[0].ConfLine->value << endl;
      }
#endif
...
#line 90 "conf_parser.yxx"
    {
      ...
      yyval.ConfLine->name = yyvsp[-3].str;
      yyval.ConfLine->value = yyvsp[-1].str;
      ;
    }
    break;
...
#line 122 "conf_parser.yxx"
    {
      ...
      if (strcmp (yyvsp[-10].str, yyvsp[-2].str) != 0)
      {
        ...
        cerr << "Brackets mismatch: Opened: " << yyvsp[-10].
          str << " Closed: " << yyvsp[-2].str << endl;
        // exit(1);
      }
      // Oll right. Append set of parameters to object($2)
      ((HtConfiguration *) YYPARSE_PARAM)->Add (yyvsp[-10].str, yyvsp[-8].str,
                                                yyvsp[-5].ConfLines);
#ifdef DEBUG
      if (sn_debug >= 2)
      {
        cout << "Added to conf: " << yyvsp[-10].str << ":" << yyvsp[-8].
          str << ":" << yyvsp[-5].ConfLines << endl;
      }
#endif
      delete yyvsp[-10].str;
      delete yyvsp[-8].str;
...
#line 143 "conf_parser.yxx"
    {
      ...
      expressionList->AddParsed (yyvsp[0].ConfLine->name,
                                 yyvsp[0].ConfLine->value);
...
#line 161 "conf_parser.yxx"
    {
      yyvsp[-1].ConfLines->AddParsed (yyvsp[0].ConfLine->name,
                                      yyvsp[0].ConfLine->value);
#ifdef DEBUG
      if (sn_debug >= 2)
      {
        cout << yyvsp[0].ConfLine->name << ":" << yyvsp[0].ConfLine->
          value << " added to " << yyvsp[-1].ConfLines << endl;
      }
#endif
      delete yyvsp[0].ConfLine->name;
      delete yyvsp[0].ConfLine->value;
...
#line 176 "conf_parser.yxx"
    {
      ...
      if ((yyval.str =
           new char[strlen (yyvsp[-1].str) + strlen (yyvsp[0].str) + 1 +
                    1]) == NULL)
      {
...
#line 228 "conf_parser.yxx"
    {
      char *old = yyval.str;
      if ((yyval.str =
           new char[strlen (yyval.str) + strlen (yyvsp[0].str) + 1 + 1]) ==
          NULL)
      {
