...
static const char *const yytname[] =
{
  ...
  "complex_expression", "simple_expression_list", "list", YY_NULLPTR
};
#endif
...
do                                                              \
  if (yychar == YYEMPTY)                                        \
    ...
    {                                                           \
      yyerror (aConf, YY_("syntax error: cannot back up")); \
      YYERROR;                                                  \
    }                                                           \
while (0)
...
static void
yy_symbol_value_print (FILE *yyoutput, int yytype, YYSTYPE const * const yyvaluep, HtConfiguration *aConf)
{
  FILE *yyo = yyoutput;
  YYUSE (yyo);
  YYUSE (aConf);
...
static void
yy_symbol_print (FILE *yyoutput, int yytype, YYSTYPE const * const yyvaluep, HtConfiguration *aConf)
{
  YYFPRINTF (yyoutput, "%s %s (",
             yytype < YYNTOKENS ? "token" : "nterm", yytname[yytype]);

  yy_symbol_value_print (yyoutput, yytype, yyvaluep, aConf);
  YYFPRINTF (yyoutput, ")");
}
...
static void
yy_reduce_print (yytype_int16 *yyssp, YYSTYPE *yyvsp, int yyrule, HtConfiguration *aConf)
{
...
do {                                    \
  if (yydebug)                          \
    yy_reduce_print (yyssp, yyvsp, Rule, aConf); \
} while (0)
...
static void
yydestruct (const char *yymsg, int yytype, YYSTYPE *yyvaluep, HtConfiguration *aConf)
{
  YYUSE (yyvaluep);
  YYUSE (aConf);
  if (!yymsg)
  ...
  if (!yyerrstatus)
    {
      ++yynerrs;
#if ! YYERROR_VERBOSE
      yyerror (aConf, YY_("syntax error"));
#else
...
`-------------------------------------------------*/
yyexhaustedlab:
  yyerror (aConf, YY_("memory exhausted"));
  yyresult = 2;
  /* Fall through.  */
#endif
