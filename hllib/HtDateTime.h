...
///////
  ...
  inline void SetDateTime (struct tm &t)
  {
    SetDateTime (&t);
  }                             // by reference
...
///////
  ...
  int GetYear () const
  {
    return (GetStructTM ().tm_year + 1900);
  }

  // Gets the month
  int GetMonth () const
  {
    return (GetStructTM ().tm_mon + 1);
  }

  // Gets the day of the week (since Sunday)
  int GetWDay () const
  {
    return (GetStructTM ().tm_wday + 1);
  }

  // Gets the day of the month
  int GetMDay () const
  {
    return GetStructTM ().tm_mday;
  }

  // Gets the day since january 1
  int GetYDay () const
  {
    return (GetStructTM ().tm_yday + 1);
  }

  // Gets the hour
  int GetHour () const
  {
    return GetStructTM ().tm_hour;
  }

  // Gets the minute
  int GetMinute () const
  {
    return GetStructTM ().tm_min;
  }

  // Gets the second
  int GetSecond () const
  {
    return GetStructTM ().tm_sec;
  }

  // Daylight saving time is in effect at that time?
  int GetIsDst () const
  {
    return GetStructTM ().tm_isdst;
  }
...
///////  //   STATIC METHODS   //  ///////
  ...
  static int DateTimeCompare (const struct tm *tm1, const struct tm *tm2);

  // Compares only date
  static int DateCompare (const struct tm *tm1, const struct tm *tm2);

  // Compares only time
  static int TimeCompare (const struct tm *tm1, const struct tm *tm2);
...
///////
  ...
  void GetStructTM (struct tm &t) const
  {
    t = GetStructTM ();
  }                             // Gets and copy
...
inline HtDateTime::HtDateTime (const HtDateTime & rhs)
{
  // Copy the contents
  Ht_t = rhs.Ht_t;
...
inline bool HtDateTime::operator== (const HtDateTime & right) const
{
  if (Ht_t == right.Ht_t)
    return true;
  else
...
inline HtDateTime & HtDateTime::operator= (const HtDateTime & right)
{
  if (this != &right)
  {
...
inline HtDateTime & HtDateTime::operator= (const int right)
{
