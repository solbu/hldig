...
//
void
Configuration::NameValueSeparators (const String & s)
{
  separators = s;
}
...
//
void
Configuration::Add (const String & str_arg)
{
...
//
void
Configuration::Add (const String & name, const String & value)
{
  String escaped;
...
//
void
Configuration::AddParsed (const String & name, const String & value)
{
...
//
int
Configuration::Remove (const String & name)
{
...
//
const String
Configuration::Find (const String & name) const
{
...
int
Configuration::Exists (const String & name) const
{
...
//*********************************************************************
Object *
Configuration::Get_Object (char *name)
{
...
//
int
Configuration::Value (const String & name, int default_value) const
{
...
//
double
Configuration::Double (const String & name, double default_value) const
{
...
//
int
Configuration::Boolean (const String & name, int default_value) const
{
  ...
  if (s[0])
  {
    if (s.nocase_compare ("true") == 0 ||
        s.nocase_compare ("yes") == 0 || s.nocase_compare ("1") == 0)
      value = 1;
    else if (s.nocase_compare ("false") == 0 ||
             s.nocase_compare ("no") == 0 || s.nocase_compare ("0") == 0)
      value = 0;
  }
...
//
const String
Configuration::operator[] (const String & name)
     const
     {
       return
       Find (name);
...
//
     int
     Configuration::Read (const String & filename)
{
  FILE *
  ...
  if (!in)
  {
    fprintf (stderr, "Configuration::Read: cannot open %s for reading : ",
             (const char *) filename);
    perror ("");
    return NOTOK;
  }
...
//
void
Configuration::Defaults (const ConfigDefaults * array)
{
